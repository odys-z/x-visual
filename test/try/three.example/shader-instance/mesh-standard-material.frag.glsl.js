...
#define TONE_MAPPING
#ifndef saturate #define saturate(a) clamp( a, 0.0, 1.0 ) #endif
...
vec3 BRDF_Specular_Sheen( const in float roughness, const in vec3 L, const in GeometricContext geometry, vec3 specularColor ) {
	...
	vec3 getLightProbeIndirectRadiance( const in vec3 viewDir, const in vec3 normal, const in float roughness, const in int maxMIPLevel ) {
		...
		float specularMIPLevel = getSpecularMIPLevel( roughness, maxMIPLevel );
			...
			#ifdef TEXTURE_LOD_EXT`
				// Ody:
				// Here's accturally where the roughtness really comes from.
				// Don't be fooled by the DRDF or GGX etc., they are not for light map.
			+ `
...
void RE_IndirectSpecular_Physical( const in vec3 radiance, const in vec3 irradiance, const in vec3 clearcoatRadiance, const in GeometricContext geometry, const in PhysicalMaterial material, inout ReflectedLight reflectedLight) {
		...
		reflectedLight.indirectSpecular += clearcoatRadiance * material.clearcoat
					* BRDF_Specular_GGX_Environment( geometry.viewDir, geometry.clearcoatNormal, vec3( DEFAULT_SPECULAR_COEFFICIENT ), material.clearcoatRoughness );
