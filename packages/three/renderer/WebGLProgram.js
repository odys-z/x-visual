...
function WebGLProgram( renderer, extensions, cacheKey, material, shader, parameters ) {
	...
	if ( material.isRawShaderMaterial ) {
		...
		prefixVertex = [
			...
			parameters.logarithmicDepthBuffer && ( parameters.isWebGL2
				/*
				 * Ody:
				 * extension['EXT_frag_depth'] is loaded by WebGLExtensions,
				 * called in WebglRenderer.initGLContext().
				 * According MDN,
				 * The EXT_frag_depth extension is part of the WebGL API and enables
				 * to set a depth value of a fragment from within the fragment shader.
				 */
				|| extensions.get( 'EXT_frag_depth' ) ) ? '#define USE_LOGDEPTHBUF_EXT' : '',
			/* Ody:
			 * This makes the shader glsl source have an extra large chunk.
			 */
