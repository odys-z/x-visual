...
export default class D3Pie extends XSys {
    ...
    constructor(ecs, options, json) {
        ...
        this.ecs = ecs;
        this.cmd = [];
        // this.camera = x.xcam.XCamera.camera;
    ...
    update(tick, entities) {
        if (!this.camera && entities.length > 0)
            this.camera = x.xcam.camera;
        for (const e of entities) {
            ...
            if (e.Pie.faceCameraXY) {
                e.Pie.norm = xmath.normxy(e.Pie.norm,
                        this.camera.position, e.Obj3.mesh.position);
                e.Ob3.mesh.setFromNormalAndCoplanarPoint(
