...
export default class D3Pie extends XSys {
	...
	static drawPie (cmpCanv, json, wh) {
		...
		var svg = d3.select("body")
					.append("div")
					// FIXME mvc
					// It is quiet clear how gpu picking is wring with position != absolute
		...
		var data = json.rows;
			...
			arc.append("path")
			   .attr("d", path)
			   // .attr("fill", function(d, i) { console.log(json.columns, d, i); return color(d.data.browser); });
...
function debugPie3(ecs, options, json) {
	...
	var p11 = ecs.createEntity({
		...
		Pie:{ faceCameraXZ: true,
			  ...
			  onClick: 1 },	 // uniform animation
		Canvas: canv,
		GpuPickable: {pickid: 1},   // FIXME mvc pickid shouldn't been set by user
		ModelSeqs: {script: [
			 [{ mtype: xv.XComponent.AnimType.ALPHA,
				paras: {
					start: Infinity,
