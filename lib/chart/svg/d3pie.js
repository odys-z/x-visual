...
export default class D3Pie extends XSys {
	...
	update(tick, entities) {
		...
		for (const e of entities) {
			...
			if (e.Pie && e.Pie.faceCameraXZ) {
				...
				*/
				e.Obj3.mesh.quaternion.copy(this.camera.quaternion);
				// e.Obj3.mesh.autoUpdateMatrix = true;
				e.Obj3.mesh.matrixAutoUpdate = true;
			}
		}
		this.cmd.splice(0);
	}
	 ...
	 * }
	 * @param {object} options {wh: { width, height }, color: ['#123456', ...]}
	...
	static pies(ecs, json, options) {
		...
		for (var pie of json) {
			...
			D3Pie.drawPie( canv, pie, {wh: canvwh, color: pie.color} );
			AssetKeepr.loadCanvtex2( canv, canvwh );
		}
