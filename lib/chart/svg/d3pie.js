...
export default class D3Pie extends XSys {
	...
	constructor(ecs, options, json) {
		...
		if (typeof options.stub !== 'string')
			throw XError('D3Pie need a div with id="stub" for inner rendering results. See docs/guid/test explained.');
		this.logged = false;
		this.ecs = ecs;
		this.cmd = [];
	...
	pies(ecs, json, options) {
		...
		for (var pie of json) {
			...
			/* Texture will be load via object of Canvas created by thrender with AssetType.DomCanvas
			...
			*/
  TODO add doc about this & opition.y
  TODO add doc about this
  TODO add doc about this
  TODO add doc about this
			// debug notes: this is called before startUpdate(),
			// it's natural in ECS that modify some option, let someothers handle it
