...
export default class XSankey extends XSys {
	...
	constructor(ecs, options, json, vectors) {
		...
		/**Bars' pivoting (extruding) positions (grid index).
		 * @member XSankey#coordmap
		 * @property coordmap
		this.coordmap = undefined; */
		...
		if (ecs) {
			...
			if (!options.grid) {
				this.grid = new ChartGrid(options.chart, json);
				options.grid = this.grid;
			}
			else this.grid = options.grid;
	 ...
	 * @return {object} {map, defs} where <br>
	 * vectorIdx: map of [vector-index, bar-entity]<br>
	...
	*/
	...
	setPivoting(coordIx, pivotings) {
		// TODO load online data
		if (!this.pivotings)
			this.pivotings = new Map();
		this.pivotings.set(coordIx, pivotings);
	}
	...
	extrudextr(e, entities) {
		if ( this.extrudingCoord >= 0 ) {
			// de-extruding
			var vix = this.pivotings[exVecs];
			// var extrudeds = this.barmap.get(vix);
			...
			// extrude
			this.extrudingCoord === e.Sankey.coordIx;
			this.getPivotings(e.Sankey, (sys, pivots) => {
				for (const e of entities) {
					var sk = e.Sankey;
					if (sys.lastExtruding !== sys.extrudingCoord) {
						var grdx = pivots[sk.vecIx][sk.coordIx];
						...
						MorphingAnim.changePosition( e.CmpTweens[2], bufArr );
					}
					e.CmpTweens.startCmds.push(1);
				}
				sys.lastExtruding = sys.extrudingCoord;
			});
		}
	}
	...
	getPivotings( sankey, onload ) {
		// TODO manage buffer
		if ( this.pivotings && this.pivotings[sankey.coordIx] )
