...
export default class XSankey extends XSys {
	...
	*/
	...
	// setPivoting(grid, entities, pivotings, bufArr) {
	// 	// var buf = [0, 0, 0];
	// }
	...
	update(tick, entities) {
		...
		for (const e of entities) {
			if (e.GpuPickable && e.GpuPickable.picked
				&& e.Sankey)
				this.extrude(e.Sankey, entities);
		}
	}
	...
	/**Extrude the selected coordinates.
	 * @param {Sankey} sankey
	 * @member XSankey.extrude
	 * @function
	 */
	extrude(sankey, entities) {
		this.getPivotings(sankey, (pivots) => {
			for (const e of entities) {
				var sk = e.Sankey;
				if (sk) {
	...
	getPivotings( sankey, onload ) {
		...
		if ( this.pivotings && this.pivotings[sankey.coordIx] )
			onload( this.pivotings.get(sankey.coordIx) );
	}
