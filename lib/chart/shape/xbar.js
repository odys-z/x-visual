...
export default class XBar extends XSys {
	...
	bars(ecs, grid, chart, vectors, options = {}) {
		...
		var offset = chart.serials ? chart.serials.offset || [0, 0, 0] : [0, 0, 0];
	...
	serials(ecs, grid, serials, options) {
		...
		// this.createLabels(l[0], posx[0][0], colors[0]);
		// var posy = grid.lerposes([l[1].length, 1, 1]);
		// this.createLabels(l[1], posy[0][0], colors[1]);
		// var posz = grid.lerposes([1, 1, l[2].length]);
		// this.createLabels(l[2], posz[0][0], colors[2]);

		var posyzx = grid.lerposes([l[0].length, l[1].length, l[2].length], [1, 1, 1]);
		// label at [x, 0, 0]
		for (var x = 0; x < posyzx[0][0].length; x++) {
			this.createLabels(ecs, l[0][x], [posyzx[0][0][x][0], 0, 0], colors[0]);
		}
		// label at [0, y, 0]
		for (var y = 0; y < posyzx.length; y++) {
			this.createLabels(ecs, l[1][y], [0, posyzx[y][0][0][1], 0], colors[1]);
		}
		// label at [0, 0, z]
		for (var z = 0; z < posyzx[0].length; z++) {
			this.createLabels(ecs, l[2][z], [0, 0, posyzx[0][z][0][2]], colors[2]);
		}
	}

	createLabels(ecs, lb, pos, style, options) {
		var box = options && options.box ? options.box : [24, 24];
