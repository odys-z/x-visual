...
export default class XBar extends XSys {
	...
	serials(ecs, grid, serials, options) {
		...
		// var box = serials["label-box"] ? serials["label-box"] : [64, 16];
		var xywh = serials["label-canvas"];
		var bg_color = serials["label-bg"] || '#003';

		// tolerate xywh.h
		if (xywh.size === undefined) {
			if (xywh.h > 0) xywh.size = xywh.h;
			else xywh.size = 32;
		}
		...
		for (var x = 0; x < posx[0][0].length; x++) {
			this.createLabels(ecs, l[0][x], posx[0][0][x],
				{ xywh, bg_color,
				  style: colors[0],
				  font: options.font});
		}
		...
		for (var y = 0; y < posy.length; y++) {
			this.createLabels(ecs, l[1][y], posy[y][0][0],
				{ xywh, bg_color,
				  style: colors[1],
				  font: options.font});
		}
		...
		for (var z = 0; z < posz[0].length; z++) {
			this.createLabels(ecs, l[2][z], posz[0][z][0],
				{ xywh, bg_color,
				  style: colors[2],
				  font: options.font});
		}
	...
	createLabels(ecs, lb, pos, options) {
		...
		var xywh = options.xywh || {x: 0, y: 0, w: options.box[0], size: options.box[1]};
		var box = [xywh.w + ( xywh.margin || 0 ) * 2, xywh.size + ( xywh.margin || 0 ) * 2];
