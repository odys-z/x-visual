...
export default class GpuPicker {
	...
	constructor(x, renderer) {
		...
		this.initMyPickings(this, x);
        me = this;
	}

	initMyPickings(me, x) {
		var scn = new THREE.Scene();
  		scn.background = new THREE.Color(0);
		me.pickingScene = scn;

		// create meshes
		var pickables = ecs.queryEntities({has});
		if (pickables) {
			pickables.forEach(function(p, ix){
			    ...
			    const picking = new THREE.Mesh(
					p.GpuPickable.geom,
					pickingMaterial);
			    pickingScene.add(picking);
			    // picking.position.copy(pickable.pos);
			    // picking.rotation.copy(pickable.rot);
			    // picking.scale.copy(pickable.scl);
			    picking.position = pickable.pos;
			    picking.rotation = pickable.rot;
			    picking.scale = pickable.scl;

				idToObject[p.id] = picking;
			});
		}
	}

	pickTest(canvPos, scene, camera, time) {
		const {pickingTexture, pixelBuffer} = this;

		// reset
		if (me.pickedObject) {
			// this.pickedObject.material.emissive.setHex(this.pickedObjectSavedColor);
			me.pickedObject = undefined;
			me.pickedId = undefined;
		}
		...
		if (intersectedObject) {
			...
			me.pickedObject = intersectedObject;
			// save its color
