...
export default class Hud extends ECS.System {
	...
	constructor(ecs, x) {
		...
		for (const hud of hudgrps) {
			if (hud.HudGroup) {
				...
				scene.background = new THREE.Color(hud.HudGroup.background || 'black');
	...
	update(tick, entities) {
		...
		for (const hudScene of this.huds)
			this.renderer.render(hudScene, this.camera);
