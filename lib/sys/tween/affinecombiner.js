...
export default class AffineCombiner extends ECS.System {
				...
				for (const twn of e.CmpTweens.tweens[seqx]) {
					...
					if (twn.affineCombine) { // flag set by animizer
						twn.onFollowed = (followee) => {
							twn._mf = new mat4(followee.mf); // FIXME at least _mf can be replaced by seq.mf
						};
						if (!e.Obj3.m0) {
							e.Obj3.m0 = new mat4();
							e.Obj3.mi = new mat4();
	...
	update(tick, entities) {
		for (const e of entities) {
			...
			if (!e.CmpTweens.idle) {
				...
				// Debug Notes:
				// About dirty
				// XTweener is not gauranteed always firing idleRising immediatly,
				...
				for (var fx = 0; fx < e.CmpTweens.endingFiring.length; fx++) {
					if (e.CmpTweens.endingFiring[fx]) {
						...
						if (e.CmpTweens.eFinished
							&& typeof e.CmpTweens.eFinished[fx] === 'function') {
							// eFinished is a copy of ModelSeqs.fFinished
							e.CmpTweens.eFinished[fx](e.CmpTweens, fx);
						}
					}
				}
			}
    ...
    /// pinned code line 100-119 - design-memo/affine.rest
