						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (mcate === AnimCate.COMBINE_AFFINE) {
								if (mtype === AnimType.ORBIT) {
									...
									var orbit = {rad: xutils.rad(scrpt.paras.deg[0]), axis: scrpt.paras.axis};
									e.Obj3.transform.push({orbit});
									e.Obj3.transform.push({translate: new vec3(scrpt.paras.pivot).arr()});
									tween = TWEEN.Tween(cmp, orbit)
										.to({rad: xutils.rad(scrpt.paras.deg[1])}, scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.ease);
								}
								else if (mtype === AnimType.ROTAXIS) {
									cmp = {affineCombine: true};
									e.Obj3.mesh.rotation.y = xutils.rad(scrpt.paras.deg[0]);
									tween = TWEEN.Tween(cmp, e.Obj3.mesh.rotation)
										.to({y: xutils.rad(scrpt.paras.deg[1])}, scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.ease);
								}
								else if (scrpt.mtype === AnimType.OBJ3ROTX) {
									cmp = {};
									e.Obj3.mesh.rotation.x = scrpt.paras.deg[0] * Math.PI / 180;
									tween = TWEEN.Tween(cmp, e.Obj3.mesh.rotation)
										.to({x: scrpt.paras.deg[1] * Math.PI / 180}, scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.ease);
								}
								// else if (mtype === AnimType.ROTATEX) {
								// 	cmp = {};
								// 	tween = TWEEN.Tween(cmp, e.Obj3.mesh.rotation)
								// 		.to({x: scrpt.paras.deg * Math.PI / 180}, scrpt.paras.duration * 1000 || cmp.duration)
								// 		.easing(scrpt.paras.ease);
								// }
								else {
