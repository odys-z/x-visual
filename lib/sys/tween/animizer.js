				...
				if (e.ModelSeqs && e.ModelSeqs.script) {
					...
					for (var seqx = 0; seqx < e.ModelSeqs.script.length; seqx++) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							else if (scrpt.mtype === AnimType.GL_VERTS_TRANS) {
								cmp = {};
								// using tween.js update shader uniforms (Obj3.uniforms)
								// TODO merge Visual.uniforms?
								// TODO doc: mpos, wpos, mrot, wrot, mscl, wscl, src, dest
								Object.assign( e.Obj3.uniforms, {tween: {mpos: 1, wpos: 1}};
								tween = TWEEN.Tween(cmp, e.Obj3.uniforms.tween)
									.to({mpos: scrpt.paras.glsl.mpos[1] || 1}, scrpt.paras.duration * 1000 || cmp.duration)
