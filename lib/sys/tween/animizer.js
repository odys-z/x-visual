...
const iffTween = ['Obj3', 'ModelSeqs', 'CmpTween'];

class MorphingAnim extends ECS.System {
	constructor(ecs, options, has) {
		super(ecs);
		this.ecs = ecs;
		var ents = ecs.queryEntities(has || {iffall: iffTween});
	...
	initTweens(ecs, entities) {
		if (entities) {
			entities.forEach(function(e, x) {
				if (e.ModelSeqs[0] && e.ModelSeqs[0][0]) {
					// e.CmpTween.tweener = XTweener(e);
                    // TODO init component here
					if (e.ModelSeqs[0][0].mtype === AnimType.OBJ3ROTX) {
						// rotate an Obj3 TODO axis
						TWEEN.Tween(e.Obj3.mesh.rotation)
...
MorphingAnim.query = { iffall: iffTween };
