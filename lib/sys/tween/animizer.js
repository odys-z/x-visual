...
class MorphingAnim extends ECS.System {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							// }
								...
								if (!e.Obj3.mesh || !e.Obj3.mesh.material) {
									...
									// should be an internal error
									throw new XError("Animizer.initTweens(): For AnimType.U_VERTS_TRANS, x-visual currently only support materials as unfiroms property, a.k.a THREE.ShaderMaterail.");
								}
								...
								if (!e.Obj3.mesh || !e.ModelSeqs.cache
									...
									// cmp = {};
									var path = e.ModelSeqs.cache; // parent line
									...
									var point;
									var {point, tween, cmp} = createFlowingParticles(path, e.FlowingPath.paras);
	...
	/** Current this doesn't acctually updating anything.
	 * @param {number} tick
	...
	/**
	 * @param {THREE.Curve} path e.g. spline supporting positionAt()
	 * @param {object} paras parameters of ModelSeqs.paras | FlowingPath.paras
	 * @return {object} {point: Mesh, tween: Tween, cmp: CmpTween}
	 * @member MorphingAnim.createFlowingParticles
	 */
	static createFlowingParticles(path, paras) {
		if (path) {
			var cmp = {};
			var {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag.throbStar, paras);
			var pointBuffer = new THREE.Vector3();
			// xglsl.pathPoints(path, paras, pointBuffer);
			cmp.onUpdateHandler = (obj, t) => {
				path.getPointAt(obj.t, pointBuffer);
			};
			...
			var point = new THREE.Mesh(pos0, mat);
			var t = {t: 0};
			var tween = TWEEN.Tween(cmp, t)
							.to({t: 1}, paras.duration * 1000 || cmp.duration)
