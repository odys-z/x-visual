...
class MorphingAnim extends ECS.System {
	...
	initTweens(ecs, entities) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							// }
								...
								var {tween, cmp} = MorphingAnim.attachShaderPosesTween(
												e.Obj3, path, e.Visual.paras.follows, scrpt.paras, e.Visual.paras);
								tween.easing(scrpt.paras.ease);
							}
							else {
		...
		function findPath(ecs, script) {
			...
			return pathent.ModelSeqs.cache.path;
		}
	}
	 ...
	 */
	static attachShaderPosTween(obj3, path, scriparas = {}, vparas = {}) {
		if (obj3 && path) {
			var cmp = {};
	...
	static attachShaderPosesTween(obj3, path, follows, scriparas = {}, vparas = {}) {
		if (obj3 && path) {
			var cmp = {};
			...
			// }
			// else {
			// 	// resverved for old version compatibility, not used
			// 	// path points should always being a Float32Array in the future
			// 	console.warn('why reach here?');
			// }
