...
class MorphingAnim extends ECS.System {
					...
					for (var seqx = 0; seqx < e.ModelSeqs.script.length; seqx++) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (mcate === AnimCate.COMBINE_AFFINE) {
								...
								// var path = e.Obj3.datum.path; // parent line
								path = findPath(ecs, scrpt);
								if (path) {
									e.Obj3.datum = Object.assign( e.Obj3.datum || {},
													{path: path.points} );
									e.Obj3.datum.dirty = true;
								}
							...
							// else if (mtype === AnimType.U_PATH_MORPHn) {
							// 	// console.log('attaching ', e);
							// 	var path = e.Obj3.datum.path; // parent line
							// 	if (path) path = path.points; 	// FXIME keeps consistance with other path branches
							// 	var {tween, cmp} = MorphingAnim.attachShaderNPosTween(
							// 					e.Obj3, path, scrpt.paras, e.Visual.paras);
							// 	tween.easing(scrpt.paras.ease);
							// }
							else if (mtype === AnimType.U_PATHn_MORPH) {
								// refence the path
								path = MorphingAnim.findPath(ecs, scrpt);
								if (path) {
									e.Obj3.datum = Object.assign( e.Obj3.datum || {},
													{path: path.points} );
									// e.Obj3.datum.dirty = false;
								}

								var {tween, cmp} = MorphingAnim.attachShaderPosesTween(
												e.Obj3, e.Visual.paras.follows, scrpt.paras, e.Visual.paras);
								tween.easing(scrpt.paras.ease);
							}
							else {
	...
	function findPath(ecs, script) {
		...
		return pathent.Obj3.datum.path;
	}
	 ...
	 */
	static attachShaderPosTween(obj3, scriparas = {}, vparas = {}) {
		if (obj3 && path) { // FIXME eliminate path argument
			var cmp = {};
				...
				cmp.onUpdateHandler = {onUpdate: function(obj, t, cmp, e) {
					if (path) {
						...
						if (e.Obj3 && e.Obj3.mesh && e.Obj3.mesh.geometry)
							...
							xgeom.getWayPointAt(p, path, obj.t);
			...
			'attachShaderPosTween(): can\'t create morphing point for undefined obj3 or path. ',
			obj3.entity);
	}

	static attachShaderPosesTween(obj3, follows, scriparas = {}, vparas = {}) {
		if (obj3.datum && obj3.datum.path) {
			...
			cmp.onUpdateHandler = {onUpdate: function(obj, t, cmp, e) {
				if (obj3.datum.path) {
					if (obj3.datum.dirty) {
						// rebuild path
						obj3.datum.dirty = false; // should we use an int flag?
					}
					...
					if (e.Obj3 && e.Obj3.mesh && e.Obj3.mesh.geometry) {
						...
						for (var ix = 0; ix < wpos.length; ix++) {
							xgeom.getWayPointAt(wpos[ix], obj3.datum.path,
