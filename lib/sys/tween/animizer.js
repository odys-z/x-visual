						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (mcate === AnimCate.COMBINE_AFFINE) {
								...
								// }
									...
									var t = {t: 0};
									var rotate = {deg:scrpt.paras.deg,axis,set:true};
									cmp.affines.push({interpos: {rotate, t}}); // vec3 is a buffer
									tween = TWEEN.Tween(cmp, t)
										.to({t: 1}, scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.ease);
								...
								// FIXME
								// FIXME Here animizer is asking xglsl that can the uniform drivable.
								...
								// FIXME shouldn't reached?
								throw new XError('FIXME shouldn\'t reach here?');
								// if (!e.Obj3.mesh || !e.Obj3.mesh.material) {
