...
import {vec3, mat4, radian} from '../../xutils/vec'
					...
					for (var seqx = 0; seqx < e.ModelSeqs.script.length; seqx++) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (mcate === AnimCate.COMBINE_AFFINE) {
								...
								if (typeof e.Obj3.affines === 'undefined')
									e.Obj3.affines = [];
								cmp = {affineCombine: true};

								if (mtype === AnimType.ORBIT) {
									var negTrans = new vec3(scrpt.paras.pivot).neg();
									e.Obj3.affines.push({translate: negTrans.arr()});
									var rotate = {rad: radian(scrpt.paras.deg[0]), axis: scrpt.paras.axis};
									e.Obj3.affines.push({rotate});
									e.Obj3.affines.push({translate: new vec3(scrpt.paras.pivot).arr()});
