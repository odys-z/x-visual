		...
		if (entities) {
			for(const e of entities) {
				...
				if(e.CmpTweens.twindx.length !== e.CmpTweens.tweens.length) {
					console.warn("Animizer.initTweens(): User definded tweens ignored as tween seq current index array lenght not equals to tweens sequences length.");
					e.CmpTweens.twindx = [];
					e.CmpTweens.tweens = [];
				}
					...
					for (var i = 0; i < e.ModelSeqs.script.length; i++) {
						...
						for (var s of seq) {
							...
							if (s.mtype === AnimType.OBJ3ROTX) {
								var cmp = {};
								var tween = TWEEN.Tween(cmp, e.Obj3.mesh.rotation)
