				...
				if (e.ModelSeqs && e.ModelSeqs.script) {
					...
					for (var seqx = 0; seqx < e.ModelSeqs.script.length; seqx++) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							var tween, cmp;
							if ((scrpt.mtype & AnimCate.COMBINE_LINEAR) === AnimCate.COMBINE_LINEAR) {
								else if ((scrpt.mtype & AnimType.MAST) === AnimType.ORBIT) {
									cmp = {};
									e.Obj3.transform = [];
									...
									throw XError("Not supported anim type (COMBINE_LINEAR === 1): ", scrpt.mtype);
								}
							}
