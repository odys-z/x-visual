						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (scrpt.mtype === AnimType.OBJ3ROTX) {
								...
								if (!e.Obj3.mesh || !e.Obj3.mesh.material) {
									console.error('Animizing vertices needing target Obj3 with mesh and material.',
										seqx, seq);
								}
								else {
									cmp = {};
									Object.assign( e.Obj3.mesh.material.uniforms,
										{tween: {twn_mpos: 0, twn_wpos: 0}});
									tween = TWEEN.Tween(cmp, e.Obj3.uniforms.tween)
										.to({ twn_mpos: scrpt.paras.glsl.mpos[1] || 1,
											  twn_wpos: scrpt.paras.glsl.wpos[1] || 1 },
											scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.glsl.ease);
									cmps.unshift(cmp);
								}
							}
							...
							var startWith = scrpt.paras.startWith;
							if (startWith) {
								// debugger
								for (var sw of startWith) {
									this.startriggerings[sw.entity] = {seqx, delay: sw.start * 1000};
								}
							}
							...
							// else if (twnx === 0) {
							// 	// or let xtweener handle this?
							// 	var starting = this.startriggerings[e.id];
							// 	if (starting && starting.seqx === seqx) {
							// 		// this entity.seq[seqx] must been started
							// 		// TODO doc: animizer
							// 		tween.start(starting.delay ? starting.delay * 1000 + TWEEN.now() : undefined);
							// 		delete this.startriggerings[e.id];
							// 	}
							// }
