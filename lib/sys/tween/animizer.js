					...
					for (var seqx = 0; seqx < e.ModelSeqs.script.length; seqx++) {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (scrpt.mtype === AnimType.OBJ3ROTX) {
								...
								cmp = {};
								// alpha is handled as uniform
								if (e.Visual && e.Visual.vtype === ShaderFlag.point
									|| e.Visual.vtype === ShaderFlag.refPoint) {
									e.Obj3.mesh.material.uniforms.u_alpha = {value: scrpt.paras.alpha[0] || 1};
									tween = TWEEN.Tween(cmp, e.Obj3.mesh.material.uniforms)
										.to({u_alpha: scrpt.paras.alpha[1] || 0.5}, scrpt.paras.duration * 1000 || cmp.duration)
										.easing(scrpt.paras.ease);
								}
								// alpha is mesh.opacity
								else {
								...
								if (!e.Obj3.mesh || !e.Obj3.mesh.material) {
									...
									// }
									xglsl.script2uniforms(scrpt.paras.uniforms, e.Obj3.mesh.material.uniforms);
									tween = TWEEN.Tween(cmp, e.Obj3.mesh.material.uniforms)
