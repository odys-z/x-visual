						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							if (mcate === AnimCate.COMBINE_AFFINE) {
								...
								if (mtype === AnimType.POSITION) {
									var translate = scrpt.paras.position;

									cmp.affines.push({translate});
									tween = TWEEN.Tween(cmp, translate)
										.to({trans: xmath.radian(scrpt.paras.deg[1])}, scrpt.paras.duration * 1000 || cmp.duration)
							...
							// }
								...
								if (!e.Obj3.mesh || !e.Obj3.mesh.material) {
									...
									// tween = TWEEN.Tween(cmp, e.Obj3.mesh.material.uniforms)
									tween = TWEEN.Tween(cmp, start)	// start is the uniform, object, a.k.a. Obj3.mesh.materail.unfiroms
								...
								if (!e.Obj3.mesh || !e.ModelSeqs.cache
									...
									var {point, tween, cmp} = MorphingAnim
										.createFlowingParticles(path, scrpt.paras, e.FlowingPath.paras);
									e.Obj3.mesh.add(point);
									// point.layers = e.Obj3.mesh.layers;
