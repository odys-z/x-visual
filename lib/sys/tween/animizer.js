...
class MorphingAnim extends ECS.System {
						...
						for (var twnx = seq.length - 1; twnx >= 0; twnx--) {
							...
							// }
								...
								if (!e.Obj3.mesh || !e.ModelSeqs.cache
									...
									// cmp = {};
									var path = e.Obj3.mesh; // parent line
	...
	static createFlowingParticles(path, paras) {
		if (path) {
			var cmp = {};
			var {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag.randomParticles, paras);
			...
			var pos0 = xglsl.pathPoint(path);
			var point = new THREE.Mesh(pos0, mat);
			var tween = TWEEN.Tween(cmp, point.material.uniforms);
