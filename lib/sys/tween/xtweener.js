	...
	update (tick, entities) {
		...
		for (const e of entities) {
			if (e.CmpTweens && e.CmpTweens.tweens) {
				...
				// clean combined buffer at each update
				// if (!e.Obj3.combined.mi)
				// 	e.Obj3.combined.mi = new mat4();
				// else e.Obj3.combined.mi.i();
				// var touchedAffines = false; // FIXME this is a strong evidence that there are something wrong in OOP design

				// if (e.Obj3.combined.isPlaying) {
				// 	e.Obj3.combined.isPlaying = false;
				// 	e.Obj3.combined.mi.i();
				// }
				...
				// 	// e.Obj3.combined.mi.put2js(m.matrix);
				// 	// m.matrix.decompose( m.position, m.quaternion, m.scale );
				// 	Affine.combineEnd(e.Obj3.combined, e.Obj3.mesh);
				// }
				// else
				// 	// this branch can't reached at the last affine updated loop
				// 	// e.Obj3.combined.m0 = undefined; // delete is meomery inefficient?
				// 	Affine.tweenStop(e.Obj3.combined);
			}
		}
	}
