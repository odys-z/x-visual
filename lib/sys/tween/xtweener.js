...
export default class XTweener extends ECS.System {
	...
	*/
		...
		if (elapsed === 1) {
			if (cmp.repeat > 0) {
				...
				if (folls && folls.length > 0) {
					...
					// }
					debugger
					XTweener.pushTriggerings(FOLLOWBY, cmp, resolvingStarts, folls);
				}
	...
	static pushTriggerings (start_or_follow, cmpStarter, resolvingBuff, withs) {
			...
			if (beingStart.triggerAt === STARTWITH && beingStart.starter.isPlaying
				...
				if (!tweens || !tweens[0]) {
					...
					if (tweens[0].startWith) {
						XTweener.pushTriggerings(STARTWITH, tweens[0], resolvingBuff, tweens[0].startWith);
					}
				}
			}
		}
...
TWEEN.Tween = function(cmpTween, tweenee) {
	...
	return new function() {
		...
		this.start = function (resolvingBuff, time) {
			...
			XTweener.pushTriggerings(STARTWITH, cmp, resolvingBuff, cmp.startWith);
			return this;
		}
