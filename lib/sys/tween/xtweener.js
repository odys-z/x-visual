...
export default class XTweener extends ECS.System {
	...
	update (tick, entities) {
					...
					if (twnx >= e.CmpTweens.tweens[seqx].length) {
						...
						if (tween && tween.isPlaying) {
							...
							if (tween.appChildUniform && e.Obj3 && e.Obj3.mesh && e.Obj3.mesh.children) {
								updateUnifroms(tween, tween.appChildUniform, e.Obj3.mesh.children);
							}
						}
						if (tween && tween.isCompleted) {
							...
							if (twnx + 1 < e.CmpTweens.tweens[seqx].length
								...
								if (typeof nxtween.onFollowed === 'function') {
									nxtween.onFollowed(tween);
								}
							}
							else if (twnx + 1 >= e.CmpTweens.tweens[seqx].length) {
								// sequence finished

								// 2020.5.26 Debug Notes
								// If finishing event fired and user handled before AffineCombiner handled
								// idleRising, the tweening matrix won't been persisted.
								// So what's here done is just setting up a firing event, let AffineCombiner
								// fire it.
								// Issue: this also add a restriction that no indepent sequnce events can
								// be fired - all concurrent sequence must fired at once.
								// This might be the essential difference that Tween.js should be rewriten.

								// if (e.CmpTweens.eFinished
								// 	&& typeof e.CmpTweens.eFinished[seqx] === 'function')
								// 	e.CmpTweens.eFinished[seqx](e.CmpTweens, seqx);
		...
		function updateUnifroms(cmp, props, meshes) {
			if (!props) return;
			...
			for (var prop of props) {
				...
				for (var mesh of meshes) {
					...
					if (mesh.children)
						updateUnifroms(cmp, props, mesh.children);
				}
			}
		}
	}
