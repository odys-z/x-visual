	...
	update (tick, entities) {
		...
		for (const e of entities) {
			...
			if (e.Obj3 && e.Obj3.mesh && e.Obj3.mesh.material
				&& e.Obj3.mesh.material.unfiorms && e.Obj3.mesh.material.uniforms.now ) {
					...
					if (twnx >= e.CmpTweens.tweens[seqx].length) {
						...
						if (tween && tween.isCompleted) {
							...
							if (twnx + 1 < e.CmpTweens.tweens[seqx].length
								...
								// If finishing event fired and user handered before AffineCombiner handled idleRising,
								// the tweening matrix won't been persisted.
								// So what's here done is just setting up a firing event, let AffineCombiner fire it.
								// Issue: this also add a restriction that no indepent sequnce events can be fired
								// - all concurrent sequence must fired at once.
