...
export default class XTweener extends ECS.System {
	...
	*/
	...
	update (tick, entities) {
		...
		for (const e of entities) {
			if (e.CmpTween) {
				...
				var startwith = this.resolvingStarts[e.id];
	...
	static startween (cmp, resolvingStarts, time, starterx ) {
		cmp.isPlaying = true;
		cmp.isPaused = false;
		...
		// added by Ody
		if (cmp.startWith) {
			var withs = cmp.startWith;
			if (withs) {
				for (var sw of withs) {
					debugger
					if (sw.starterx.seqx === starterx.seqx
						&& sw.starterx.twnx === starterx.twnx) {
...
TWEEN.Tween = function(cmpTween, tweenee) {
	const cmp = cmpTween;
	// const resolvingBuff = resolvingStarts || {};
	cmp.duration = 1000;
	cmp.repeat = 0;
	cmp.delayTime = 0;
	cmp.startTime = null;
	...
	return new function() {
		...
		this.start = function (resolvingBuff, time) {
			XTweener.startween(cmp, resolvingBuff, time,
				cmp.startWith ? cmp.startWith.starterx : undefined);
			return this;
		}
