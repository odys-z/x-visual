				...
				for (var seqx = 0; seqx < e.CmpTweens.twindx.length; seqx ++) {
					...
					if (tween && tween.isCompleted) {
						...
						if (twnx + 1 < e.CmpTweens.tweens[seqx].length
							...
							XTweener.startween(e.CmpTweens.tweens[seqx][twnx + 1],
								undefined, this.resolvingStarts);
						}
					}
					...
					if (beingStartwiths) {
						...
						for (var i = 0; i < beingStartwiths.length; i++) {
							...
							if (beingStart.starter.isPlaying) {
								...
								if (!tweens || !tweens[0]) {
									...
									XTweener.startween(tweens[0], this.resolvingStarts, tweens[0].delay);
									tweens.twindx[beingStart.seqx] = 0;
									beingStartwiths.splice(i, 1);
								}
							}
						}
						if (this.resolvingStarts[e.id].length === 0)
