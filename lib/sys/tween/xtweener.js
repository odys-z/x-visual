...
export default class XTweener extends ECS.System {
	...
	start(time) {
		...
		this.startTime = time !== undefined ? typeof time === 'string' ? TWEEN.now() + parseFloat(time) : time : TWEEN.now();
	...
	pause(time) {
		...
		this.isPaused = true;
		this.pauseStart = time === undefined ? TWEEN.now() : time;
		this.group.remove(this);
		return this;
	}
	...
	resume(time) {
		...
		this.isPaused = false;
		this.startTime += (time === undefined ? TWEEN.now() : time)
						- this.pauseStart;
		this.pauseStart = 0;
		this.group.add(this);
		return this;
