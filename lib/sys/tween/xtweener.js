...
export default class XTweener extends ECS.System {
	...
	*/
	...
	update: function (tick, entities) {
		time = x.lastUpdate;
		entities.forEach(function(e, ix) {
			var tween = e.CmpTween;
			if (tween && tween.isPlaying) {
				tween.isPlaying = updateTween(tween, time);
				if (e.preserve === false) {
					// delete this._tweens[tweenIds[i]];
					// what's here?
					// e.deprecated = true;
...
class XTweener {
	Tween(cmpTween) {
		...
		return new function() {
			getId() { return cmp.id; }
			isPlaying() { return cmp.isPlaying; }
			isPaused() { return cmp.isPaused; }

			to(cmp, properties, duration) {
