...
export default class XTweener extends ECS.System {
	...
	*/
		...
		for (const e of entities) {
			if (e.CmpTween) {
				...
				for (var seqx = 0; seqx < e.CmpTweens.twindx.length; seqx ++) {
					...
					if (beingStartwiths) {
						var slicings = [];
						for (var i = 0; i < beingStartwiths.length; i++) {
							var beingStart = beingStartwiths[i];
							if (beingStart.triggerAt === STARTWITH && beingStart.starter.isPlaying
								|| beingStart.triggerAt === FOLLOWBY && beingStart.starter.isCompleted) {
								var tweens = e.CmpTweens.tweens[beingStart.seqx];
								if (!tweens || !tweens[0]) {
									console.error('XTweener.update(): StartWith script index is out of range.',
										'Index:\n', beingStart.seqx, 'Tweens:\n', e.CmpTweens.tweens);
								}
								else if (!tweens[0].isPlaying && !tweens[0].isCompleted) {
									XTweener.startween(tweens[0], tweens[0].delay);
									e.CmpTweens.twindx[beingStart.seqx] = 0;
									// beingStartwiths.splice(i, 1);
									slicings.push(i);
								}
							}
						}
						if (slicings.length > 0) {
							for (var i = slicings.length - 1; i >= 0; i--)
								beingStartwiths.splice(slicings[i], 1);
						}
					}
				}
		...
		if (elapsed === 1) {
			if (cmp.repeat > 0) {
				...
				if (folls && folls.length > 0) {
				 	// for (var fll of folls) {
					// 	console.log(fll);
					// 	if (fll && !resolvingStarts[fll.entity]) {
					// 		resolvingStarts[fll.entity] = {
					// 			seqx: fll.seqx, delay: fll.satart};
					// 	}
					// }
					debugger
					XTweener.pushTriggereds(FOLLOWBY, cmp, resolvingStarts, folls);
				}
	...
	static pushTriggereds (start_or_follow, cmp, resolvingBuff, withs) {
	...
	// static startWithTween(withx, starterx) {
	// 	if (withx) {
	// 		for (var wx = 0; wx < withx.length; wx++) {
	// 			debugger
	// 			w = withx[wx];
	// 			if (w.starterx.seqx !== starterx.seqx
	// 				|| w.starterx.twnx !== starterx.twnx)
	// 				continue;
	//
	// 			resolvingStarts[sw.entity] = {
	// 				seqx: sw.seqx, delay: sw.start * 1000,
	// 				starterx: sw.starterx };
	// 			withs.remove(sw);
	// 		}
	// 	}
	// }
...
TWEEN.Tween = function(cmpTween, tweenee) {
	...
	return new function() {
		...
		this.start = function (resolvingBuff, time) {
			...
			if (cmp.startWith) {
				// we don't have the entity's CmpTweens here, can only start it in update()
				for (var wx of cmp.startWith) {
					wx.starter = cmp;
					if (resolvingBuff[wx.entity] === undefined)
					 	resolvingBuff[wx.entity] = [];
					resolvingBuff[wx.entity].push(wx);
				}
				// delete cmp.startWith;
			} */
			XTweener.pushTriggereds(STARTWITH, cmp, resolvingBuff, cmp.startWith);
			return this;
		}
