	...
	*/
		...
		for (const e of entities) {
			if (e.CmpTween) {
				...
				if (tween && tween.isPlaying) {
					tween.isPlaying = XTweener.updateTween(tween, time);
					if (e.preserve === false) {
						// delete this._tweens[tweenIds[i]];
						// what's here?
					}
				...
				for (var ix = 0; ix < e.CmpTweens.twindx.length; ix ++) {
					if (ix >= e.CmpTweens.tweens.length)
						// FIXME we need some flag to stop empty runing if all animation finished
						continue;

					var twx = e.CmpTweens.twindx[ix];
					if (twx >= e.CmpTweens.tweens[ix].length) {
						console.error(`XTweener.update(): Why try to update ${ix}-th tween of`,
							e.CmpTweens.tweens, "\nEntity: ", e);
						continue;
					}
					const tween = e.CmpTweens.tweens[ix][twx];
					if (tween && tween.isPlaying) {
						tween.isPlaying = XTweener.updateTween(tween, time);
						if (e.preserve === false) {
							// what's here?
					...
					else if (tween && tween.isCompleted) {
						// finished, start next
						e.CmpTweens.twindx[ix] = twx + 1;
						if (twx + 1 < e.CmpTweens.tweens[ix].length
							&& e.CmpTweens.tweens[ix][twx + 1])
							XTweener.startween(e.CmpTweens.tweens[ix][twx + 1]);
					}
				}
			}
		}
	...
	static startween (cmp, time) {
		...
		for (var property in cmp.valuesEnd) {
			// Check  cucgusaucsdoiuvhj'kl
		]\ an Array was provided as property value
 ...
 * @param {object} tweenee object to be tweened
 * */
TWEEN.Tween = function(cmpTween, tweenee) {
	...
	// Because of this, CmpTween been mixed in too many methods, the whole Tween.
	// We need a better design pattern to handle this problem.
	// The problem currently is, if wee don't store tween instance, we can't
	// deprecate chainedTweens - we need it to schedule tweens later when a tween
	// is finished
	// cmp.tween = this;
...
kds/lkiuo][=;[-0polmklcxwqzexrcW]]
// import {version} from '../.temp.version.js';Ss
TWEEN.version = '0.2.0';

export { TWEEN, XEasing };
+46
.
 jciujkqwecbx;iu g;oii2oii2Howeversdaxzihql
