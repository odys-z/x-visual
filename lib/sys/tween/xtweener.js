	...
	update (tick, entities) {
		...
		for (const e of entities) {
			if (e.CmpTweens && e.CmpTweens.tweens) {
				if (e.CmpTweens.startCmds.length > 0) {
					for (var cmdx of e.CmpTweens.startCmds) {
						if (0 <= cmdx && cmdx < e.CmpTweens.tweens.length) {
							...
							// starting snapshot
							// e.Obj3.transform = mat4.js(e.Obj3.mesh.matrix);

							// here or statTween()? what about startWith?
							// if (!e.Obj3.combined.m0)
							// 	e.Obj3.combined.m0 = mat4.js(e.Obj3.mesh.matrix);
							// Affine.startTweens(e.Obj3.combined);
						}
					}
					...
					if (twnx >= e.CmpTweens.tweens[seqx].length) {
						...
						if (tween && tween.isPlaying) {
							...
							if (tween.affineCombine) {
								/*
								e.Obj3.combined.isPlaying = true;
								var m = e.Obj3.mesh; // for short

								// take snapshot - most scripts are supposed to have affines
								if (!e.Obj3.combined.m0) {
									// m.updateMatrix();
									// e.Obj3.combined.m0 = new mat4(m.matrix)
									// 						.transpose();
									// e.Obj3.combined.mi = new mat4();
									Affine.startTweens(e.Obj3.combined);
								}
								...
								Affine.combine(tween.affines, e.Obj3.combined);
								* /
								Affine.combineUpdate(tween.affines, e.Obj3.combined, e.Obj3.mesh);
							}
						}
						if (tween && tween.isCompleted) {
							...
							if (twnx + 1 < e.CmpTweens.tweens[seqx].length
								...
								// debug notes:
								// Affine snapshot for m0 here is still a bug:
								// If another tween sequence finished, say tweens[1][0] (length = 1)
								// it has no chance to push it's results for further tweening.
								// - [1][0].mi lost it's results as it's not pushed into m0
								if (nxtween.affineCombine) {
									// keep snapshot m0
									// touchedAffines = true;
									e.Obj3.combined.isPlaying = true;
									e.Obj3.combined.m0 = new mat4(e.Obj3.mesh.matrix);
								}
							}
						}
					}
				...
				Affine.combineEnd(e.Obj3.combined, e.Obj3.mesh); // mi combining finished
				if (!e.Obj3.combined.isPlaying) // reset by combineBegin(), flipped by combineUpdate()
					// bug: this branch will reached and m0 been cleared
					// because when tweens[0][0] finished - which will update affines and tweens[1][0] not started,
					// then the e.Obj3.combined.isPlaying == false (reset before the for loop)
					Affine.tweenStop(e.Obj3.combined, e.Obj3.mesh);
			}
		}
	}
	*/
