...
// const __tex0__ = GlUniform.tex0(); - why DataTexture won't work?
...
export default class Thrender extends ECS.System {
	...
	static createObj3s(ecs, scene, light, entities) {
		...
		entities.forEach(function(e, x) {
			...
			if ( ! e.Visual.material && e.Visual.shader
				...
				uniforms = GlUniform.init(e.Visual, e.Obj3, light, ecs);
			}
					...
					if (!e.Visual.material) {
						...
						if (e.Visual.shader) {
							...
							def = Object.assign( def, {
									opacity: alp,
									// transparent: false, // FIXME change back true,FIXME change back true,FIXME change back true,
									transparent: true,
									side,
									shadowSide,
									depthWrite: true,
	...
	static xlineMaterial( uniforms = new Object(), vparas ) {
		...
		// use xline to control u_alpha
			...
			var matOpt = {	uniforms,
							...
							fragmentShader,
							depthTest: true,
							// transparent: false, //true, FIXME change back  FIXME change back FIXME change back
							transparent: true,
							depthWrite: true };
	...
	static createObj3particles(cmpObj3, fromesh, cmpVisual) {
		...
		var material = Thrender.createXShaderMaterial(cmpVisual.shader, uniforms, cmpVisual.paras, cmpObj3);
	...
	init(canvas, x) {
		...
		this.mrt = new THREE.WebGLMultiRenderTarget(
			// wh[0] * window.devicePixelRatio,
			// wh[1] * window.devicePixelRatio,
		...
		// this.filterScene.add(light); // Filterenderer shouldn't change anything
		...
		let {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag._hBlur,
			x.options.light, undefined, !!(x.options.finalQuad && x.options.finalQuad.debug));
		...
		let filterUniforms = Object.assign( {
				...
				u_blurRadius: { value: x.options.finalQuad
					&& x.options.finalQuad.blurRadius !== undefined
					? x.options.finalQuad.blurRadius : 4 },
				u_flareUv: { value: new THREE.Vector2(1.0) },
				u_flareHue: { value: new THREE.Vector3(...glConfig.xfilter.flareHue) },
				u_time: { value: 1 },
				u_testPos: { value: new THREE.Vector3() },
				u_testex: { vlaue: __tex0__ },
				// u_testex: { vlaue: AssetKeepr.cheapixelTex() },
			...
			// , THREE.ShaderLib.shadow.uniforms );
		...
		let finalShader = xglsl.xvShader(ShaderFlag._finalQuad,
			undefined, undefined, !!(x.options.finalQuad && x.options.finalQuad.debug));
		this.postScene.add( new THREE.Mesh(
			...
			new THREE.RawShaderMaterial( {
				...
				uniforms: {
					...
					xColor: { value: this.mrt.textures[ 1 ] },
					xEnvSpecular: { value: this.mrt.textures[ 2 ] },
					xOutline: { value: this.filterBuffer.textures[ 0 ] },
					xBlurH: { value: this.filterBuffer.textures[ 1 ] },
					u_texsize: { value: new THREE.Vector2(...wh) },
				},
				lights: false
			} )
	...
	update(tick, entities) {
		...
		// this._v3.applyMatrix3( this.camera.matrixWorldInverse ).normalize()
		// 		.applyMatrix3( this.camera.projectionMatrix ).normalize();
		// this._v3 = vec3.dir2Uv(this._v3, camera);
		...
		this.filterQuad.material.uniforms.directionalShadowMatrix.value = [this.light.shadow.matrix];
		this.filterQuad.material.uniforms.directionalShadowMap.value[0] = this.light.shadow.map.texture;
		if (x.light.flareHue) {
			this.filterQuad.material.uniforms.u_flareHue.value.x = x.light.flareHue[0];
			this.filterQuad.material.uniforms.u_flareHue.value.y = x.light.flareHue[1];
			this.filterQuad.material.uniforms.u_flareHue.value.z = x.light.flareHue[2];

			/*
			this.filterQuad.material.uniforms.u_testPos.value.x = Math.random();
			this.filterQuad.material.uniforms.u_testPos.value.y = Math.random();
			this.filterQuad.material.uniforms.u_testPos.value.z = Math.random();
			*/
			this.filterQuad.material.uniforms.u_testPos.value = this.camera.position;
			this.filterQuad.material.uniforms.u_testex.value = __tex0__;
