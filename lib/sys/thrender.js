...
export default class Thrender extends ECS.System {
	...
	static createLight (options = {}) {
		...
		options.diffuse = xutils.cssColor(sky);
		options.ambient = xutils.cssColor(ground);
	 ...
	 * FIXME Should we need a scene class to manage objects?
	 *
	 * @param {string} eid
	 * @param {object} vparas
	 * @param {object} light
	 ...
	 * @member Thrender.updateMaterial
	 * @function
	 */
	static updateMaterial(eid, vparas = {}, light = {}, uniforms = {}) {

		// let updateUniforms = {};
		let e = this.ecs.getEntity(eid);

		if(e) {
			// todo parse param;
			// updateUniforms = xglsl.formatUniforms(VisualPara, e.Obj3, light);
			GlUniform.update(uniforms, vparas, light);

			let mesh = e.Obj3.mesh;
			if(mesh) {
				let material = mesh.material;
				let matUniforms = material.uniforms;
				Object.assign(updateUniforms, uniforms);
				Object.assign(matUniforms, updateUniforms);
	...
	init(canvas, x) {
		...
		// light
		var light = Thrender.createLight(x.options.light);
		x.light = light;
		scene.add(light);
