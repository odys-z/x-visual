...
export default class Thrender extends ECS.System {
	...
	static createXShaderMaterial( shaderflag, uniforms = {}, vparas, obj3 ) {
		if (shaderflag === ShaderFlag.colorArray) {
			...
			// TODO merge with XMaterial.init()
	...
	static create3Material(uniforms = {}, vparas, obj3) {
		let def = {};
		if (e.Visual.asset)
			def.map = AssetKeepr.loadTexure( e.Visual.asset, function() {
				let def_ = def;
		...
		else // default and for asynchronous
			def.map = new ramTexture(3, 4, {alpha: alp});

		let side = !e.Visual.paras || e.Visual.paras.side === undefined
					? THREE.DoubleSide : e.Visual.paras.side;
		let alphaMap = e.Visual.paras && e.Visual.paras.enableAlphaMap ?
					def.map : undefined;
		let blending = e.Visual.paras && e.Visual.paras.blending !== undefined ?
					e.Visual.paras.blending : undefined;
		...
		if (e.Visual.vtype === AssetType.mesh)
			mat = new THREE.MeshPhongMaterial(def);
		else if (e.Visual.vtype === AssetType.skyBox) {
			mat = new THREE.MeshPhongMaterial(def);
		}
		else // mesh_basic
			mat = new THREE.MeshBasicMaterial(def);
