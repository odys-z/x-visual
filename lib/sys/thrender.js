...
export default class Thrender extends ECS.System {
	...
	init(canvas, x) {
		...
		if ( x.options.ssaoRadius > 0 && x.options.ssaoIntense > 0) {
			filterDefaults.u_ssaoRadius = x.options.ssaoRadius;
			filterDefaults.u_ssaoIntense = x.options.ssaoIntense;
		}
		...
		let finalShader = xglsl.xvShader(
			x.options.finalQuad && x.options.finalQuad.shader ?
				x.options.finalQuad.shader : ShaderFlag._finalQuad,
			undefined, undefined,
			!!(x.options.finalQuad && x.options.finalQuad.debug));
