...
export default class Thrender extends ECS.System {
	...
	init(canvas, x) {
		...
		let filterDefaults = {
				u_blurRadius: finalOpt.blurRadius !== undefined ? finalOpt.blurRadius : 4,
				u_blurIntense: finalOpt.blurIntense !== undefined ? finalOpt.blurIntense : 1.,
			  };

		if ( finalOpt.ssaoRadius > 0 && finalOpt.ssaoIntense > 0) {
			filterDefaults.u_ssaoRadius = finalOpt.ssaoRadius;
			filterDefaults.u_ssaoIntense = finalOpt.ssaoIntense;
		}
		...
		let finalShader = xglsl.xvShader( finalOpt.shader ?
				...
				undefined, undefined, !!finalOpt.debug);
