...
export default class Thrender extends ECS.System {
	 ...
	 * @param {object} vpara Visual.paras - shouldn't here as this only care about geometry.
	 * @member Thrender.createObj3mesh
	 * @function
	 */
	static createObj3mesh(cmpObj3, geomType, material, vpara) {
		if (cmpObj3) {
			var g = Thrender.threeGeometryCase(geomType, cmpObj3.box, vpara.points);
	...
	static threeGeometryCase(geomType, geomParaArr, vparas) {
		...
		if (geomType === Obj3Type.BOX) {
			...
			g = geom.generateWayxz(points, geomParaArr[0]);
		}
		else {
			if (y > 5) {
