...
export default class Thrender extends ECS.System {
	...
	init(canvas, x) {
		...
		let filterUniforms = Object.assign( {
				...
				directionalLights: { value: [{
					color: light.color,
					direction: light.position.normalize(),
					intensity: light.intensity }] },
				u_time: { value: 1 },
				u_campos: { value: new THREE.Vector3() },
				u_camlook: { value: new THREE.Vector3(0, 0, -1) }, // dir
	...
	update(tick, entities) {
		...
		if (x.light.flare > 0) {
			...
			uniforms.directionalLights.value[0].derection = this.light.position.normalize();
