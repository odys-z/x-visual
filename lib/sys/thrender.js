...
export default class Thrender extends ECS.System {
			...
			switch (e.Visual.vtype) {
				...
				case AssetType.mesh_basic:
					...
					if (!e.Visual.material) {
						...
						if (e.Visual.shader) {
							...
							// FIXME time to reorgnize uniforms handling, see PathTube
							// FIXME in this branch, should get defualt uniforms from glsl?

							/** NOTE before deleting this, you also need use glsl.formatUniforms() for other branches
				...
				case AssetType.point:
					...
					if (!e.Visual.asset) {
						...
						addObj3(scene, m, e);	// 2020.05.28 BUG? Not mesh?
					}
				...
				case AssetType.PathTube:
					...
					break;
				case AssetType.vertParticle:
					//console.log("vertParticle")
					var mesh = Thrender.createPracle(e.Obj3, e.Visual);
					addObj3(scene, mesh, e);
					break;
	 ...
	 * @function
	 */
	static createXShaderMaterial( shaderflag, uniforms = {}, vparas ) {
	 ...
	 * @function
	 */
	static createObj3mesh(cmpObj3, geomType, material, visual, modelSeqs) {
		if (cmpObj3) {
			var g = Thrender.threeGeometryCase(geomType, cmpObj3.box,
							visual ? visual.paras : undefined, modelSeqs);
	 ...
	 * @function
	 */
	static threeGeometryCase(geomType, geomParaArr, vparas, modelSeqs) {
		...
		if (geomType === Obj3Type.BOX) {
			...
			if (vparas && vparas.features) {
				var {geom, path} = xgeom.generateWayxz( vparas.features,
					  ...
					  vparas ? vparas.style : undefined );
				g = geom;
			}
			else if (vparas && vparas.uri){
				// first create a stub path
				var {geom, path} = xgeom.generateWayxz( [{
						type: "Feature",
						geometry: {
							"type": "LineString",
							"coordinates": [ [0,0] ] } }],
				...
				g = geom;
				// load from uri
				AssetKeepr.loadJson(vparas.uri, (json) => {
					var {geom, path} = xgeom.generateWayxz( json.features,
						  vparas ? vparas.y0 || geomParaArr[0] || 0 : geomParaArr[0] || 0,
						  vparas ? vparas.origin : undefined,
						  vparas ? vparas.style : undefined );
					g.vertices = geom.vertices;
 ...
 /**
     * particles
     */
    static createPracle(cmpObj3, cmpVisual) {
            ...
            var particles = cmpVisual.paras && cmpVisual.paras.particlesNum ? cmpVisual.paras.particlesNum : 100;
            var ppos = cmpVisual.paras && cmpVisual.paras.positions ? cmpVisual.paras.positions : null;
            for (var i = 0; i < particles; i++) {
                if (Array.isArray(ppos)) {
                    positions.push(ppos[i * 3]);
                    positions.push(ppos[i * 3 + 1]);
                    positions.push(ppos[i * 3 + 2]);
            ...
            if (cmpVisual.paras && cmpVisual.paras.u_tex) {
                var uniforms = {
                    pointTexture: { value: new THREE.TextureLoader().load(cmpVisual.paras.u_tex) }
                };
