...
var __renderer__120__; // for debug
...
export default class Thrender extends ECS.System {
	...
	static createXShaderMaterial( shaderflag, uniforms = {}, vparas, obj3, ecs ) {
		...
		let mat = new THREE.RawShaderMaterial// ShaderMateriall
		( {
		...
		if (vtype === AssetType.mesh)
			...
			if (Array.isArray(u_tex) && u_tex.length > 1) {
				...
				if (AssetKeepr.isExr(pth)) { // exr
					var pmremGenerator = new PMREMGenerator( __renderer__120__ );
						...
						.load(
							...
							function() {
								...
								return function ( texture ) {
									...
									let rt = new THREE.WebGLCubeRenderTarget(texture.image.height, {generateMipmaps: true});
									rt.fromEquirectangularTexture(__renderer__120__, texture);
									texture.dispose();
									if (_scene)
									...
									if (_obj3.datum.registers) {
										for (let mt of _obj3.datum.registers) {
											// experiments shows both material.envMap and uniforms.envMap need to be set
											// https://jsfiddle.net/p23rd7ft/10/
											// https://jsfiddle.net/684vtyoz/
											// https://discourse.threejs.org/t/solved-how-to-use-envmap-in-shadermaterial-based-on-meshphongmaterial/13003/4
											// so set envMap
											mt.envMap = rt.texture;
											mt.uniforms.envMap = { value: rt.texture };
											mt.needsUpdate = true;
										}
									}
								}
					...
					let texture = loader.load(
						...
						function() {
							...
							return function ( rt ) {
								...
								// Design Memo: referencing object should use this by itself as an equirectangle
								rt.isEquirect = true;
								rt.minFilter = THREE.NearestFilter;
								rt.maxFilter = THREE.NearestFilter;
								// rt.generateMipmaps = false;
	 ...
	 * @param {object} [options.shadow.mapSize={width: 1024, height: 1024}]
	...
	static createLight (options = {}, cam = {}) {
		...
		light.shadow.camera = Object.assign(light.shadow.camera,
			Object.assign(
				{ near: cam.near == undefined ? 1 : cam.near,
				  far: cam.far == undefined ? 2000 : cam.far },
				{ near: s.near, far: s.far} )
		);
	...
	init(canvas, x) {
		...
		this.mrt = new THREE.WebGLMultiRenderTarget(
			...
			2,
			{
				...
				generateMipmaps: false, // FIXME not work?
				stencilBuffer: false,
				depthBuffer: true
			}
		);
		...
		let {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag._finalQuad, undefined, undefined, true);
	...
	update(tick, entities) {
		...
		else // with outlinePass, and more like SSOA.
			// We will drpecationg post effects
