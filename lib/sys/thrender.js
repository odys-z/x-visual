...
export default class Thrender extends ECS.System {
				...
				case AssetType.mesh_basic:
					if (!e.Visual.material) {
						...
						if (e.Visual.shader) {
							...
							Thrender.resolveEnvMap(e.Visual.paras, mat);
						}
					}
	...
	static resolveEnvMap (vparas, mat) {
	 ...
	 * @function
	 */
	static createXShaderMaterial( shaderflag, uniforms = {}, vparas, obj3 ) {
		...
		Thrender.resolveEnvMap(vparas, mat);
		return mat;
	}
	...
	static create3Material(uniforms = {}, vtype, vparas, obj3, scene) {
		...
		if (u_tex) {
			// TODO FIXME for debug, merge with sky box
			// TODO FIXME for debug, merge with sky box
			// TODO FIXME for debug, merge with sky box
			// def.map = AssetKeepr.loadTexure( u_tex, function() {
			// 	let def_ = def;
			// 	return (texture) => {
			// 		texture.needsUpdate = true;
			// 		def_.map = texture;
							...
							return function ( texture ) {
								...
								if (_obj3.datum.registers) {
									for (let mt of _obj3.datum.registers) {
										...
										// if (mt.isShaderMaterial)
										// 	mt.uniforms.envMap = { value: texture };
										// else
											mt.envMap = rt.texture;
											// mt.uniforms.envMap = { value: texture };
											mt.uniforms.envMap = { value: rt.texture };
											// mt.combine = THREE.MultiplyOperation;
											// mt.uniforms.envMapIntensity = { value: 1.};
										// mt.lights = true
										mt.needsUpdate = true;
									}
								}
