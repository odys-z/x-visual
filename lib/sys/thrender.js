...
export default class Thrender extends ECS.System {
			...
			if (e.Visual.vtype === AssetType.gltf) {
				...
				if (e.Visual.vtype === AssetType.canvas) {
					...
					if (e.Visual.asset)
						...
						// tex = new ramTexture(1, 1, {r: 0.2, g: 0.3, b: 0.5, a: alp});
						tex = new ramTexture(1, 1, alp);
					mat = new THREE.MeshBasicMaterial({
							map: tex,
							opacity: alp,
							transparent: true,
	...
	static createObj3particles(cmpObj3, fromesh, cmpVisual) {
		...
		if ((cmpVisual.paras.dest || cmpVisual.paras.a_dest)
			...
			var geometry = xglsl.particlesGeom(cmpVisual.shader, cmpVisual.paras, m0,
	...
	static createCubeVoxel(cmpObj3, cmpVisual) {
		...
		// var {vertexShader, fragmentShader} = xglsl.xvShader(ShaderFlag.testPoints, cmpVisual.paras);
		var {vertexShader, fragmentShader} = xglsl.xvShader(cmpVisual.shader, cmpVisual.paras);

		// FIXME
		// FIXME
		// FIXME
		// FIXME is this means Obj3.uniforms is othogonal to Visual.shader?
		// var shadr = cmpVisual.shader & ShaderFlag.mask;
		// if (shadr === ShaderFlag.randomParticles
		// 	|| shadr === ShaderFlag.testPoints) {
		if (cmpObj3.uniforms)
			Object.assign(uniforms, xglsl.obj2uniforms(cmpVisual.paras.uniforms));
		...
		for (var bx = 0; bx < cmpVisual.paras.uniforms.u_cubes.length; bx++) {
			...
			if (destcube && (!entCube || !entCube.Obj3.mesh || !entCube.Obj3.mesh.geometry.attributes['position'])) {
				...
				if (x.log >= 3)
					console.warn(`[log 3] Can't create cube voxel from geometry other than box. Visual.paras.a_cube = ${destcube}`);
			}

			var u_boxi = `u_box${bx}`;
			var u_transi = `u_trans${bx}`;
			uniforms[u_boxi] = {value: xutils.vec3(entCube.Obj3.box)};
			uniforms[u_transi] = {value: xutils.mat4(entCube.Obj3.mesh.matrix)};
			// and animizer will add u_morph1/2
		}
