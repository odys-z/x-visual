...
export default class Thrender extends ECS.System {
	...
	constructor(ecs, x) {
		...
		this.light = Thrender.createLight(x.light); // TODO probably we need another system
		...
		Thrender.createObj3s(ecs, scn, this.light.paras, obj3Ents);
	}
	...
	static geometryCase(geomType, geomParaArr, vparas, obj3) {
		...
		if (geomType === Obj3Type.BOX) {
			...
			// synchrodous
			if (vparas && vparas.features) {
				var opts = Thrender.formatPrismOption(obj3, vparas);
	...
	static createLight (options = {}) {
		...
		var sky = options.skyColor || 0xffffbb;
		var ground = options.groundColor || 0xffffbb;

		// TODO move to vec3
		var hex = Math.floor( sky );
		options.u_diffuse = [(hex >> 16 & 255 ) / 255,
							( hex >> 8 & 255 ) / 255,
							( hex & 255 ) / 255 ];

		hex = Math.floor( ground );
		options.u_ambient = [(hex >> 16 & 255 ) / 255,
							( hex >> 8 & 255 ) / 255,
							( hex & 255 ) / 255 ];
		options.u_lightIntensity = options.intensity || 1.1;
		...
		light = new THREE.HemisphereLight( sky, ground, options.u_lightIntensity );
