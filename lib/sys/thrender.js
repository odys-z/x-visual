...
export default class Thrender extends ECS.System {
			...
			switch (e.Visual.vtype) {
				...
				case AssetType.mesh_basic:
					...
					if (!e.Visual.material) {
						// // implied by Dynatex needing a colorArray
						// if (!e.Visual.shader && e.Dynatex) {
						...
						if (e.Visual.shader) {
							...
							// 	if (e.Visual.paras && e.Visual.paras.u_texWeight !== undefined) {
							// 		uniforms.u_texWeight = {value: e.Visual.paras.u_texWeight};
							// 	}
							//
							// 	if (e.Visual.paras && e.Visual.paras.colors && e.Visual.paras.colors.length > 0) {
							// 		var clrs = [];
							// 		for (var i = 0; i < e.Visual.paras.colors.length; i++) {
							// 			clrs.push( new THREE.Vector3(...e.Visual.paras.colors[i]) );
							// 		}
							// 		uniforms.u_color = {value: clrs};
							// 	}
							// 	if (e.Visual.paras && e.Visual.paras.u_tex && e.Visual.paras.u_tex.length > 0) {
							// 		var texs = [];
							// 		for (var i = 0; i < e.Visual.paras.u_tex.length; i++) {
							// 			tex = new ramTexture(i+1, i+1, {alpha: alp});
							// 			// suppres RanderPass warning
							// 			tex.needsUpdate = false;
							// 			texs.push(tex);
							//
							// 			AssetKeepr.loadTexure(e.Visual.paras.u_tex[i],
							// 				(texture) => {
	...
	static createXShaderMaterial( shaderflag, uniforms = {}, vparas ) {
		...
		// else throw new XError('so what?', 0);
