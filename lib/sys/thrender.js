...
export default class Thrender extends ECS.System {
				...
				if (e.Visual.vtype === AssetType.canvas) {
					...
					// xglsl create Obj3 particles
						...
						if (e.Visual.vtype === AssetType.refPoint)
							...
							pointMesh = Thrender.createCubeVoxel(e.Obj3, eref.Obj3.mesh, e.Visual);
	...
	static createCubeVoxel(cmpObj3, cmpVisual) {
		...
		for (var bx = 0; bx < cmpVisual.paras.u_cubes.length, bx++) {
			if (!destcube) {
			...
			Object.assign(uniforms, { `u_box${bx}`: {value: new xutils.Vector3(entCube.Obj3.box)} });
			// and animizer will add u_morph1/2 and u_trans1 / 2
		}
		...
		var geometry = xglsl.cubeVoxelGeom(cmpVisual.paras);
		var mesh = new THREE.Points( geometry, material );
		return mesh;
	}
