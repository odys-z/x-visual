...
export default class Thrender extends ECS.System {
			...
			switch (e.Visual.vtype) {
				...
				case AssetType.mesh_basic:
					if (!e.Visual.material) {
						...
						if (e.Visual.shader) {
							...
							if (e.Visual.asset)
								tex = AssetKeepr.loadTexure( e.Visual.asset ); // FIXME it's excpected working in asynchronous
				...
				case AssetType.DynaSects:
					...
					if (e.ModelSeqs) {
						...
						if (!e.Obj3.datum)
							e.Obj3.datum = new Object();
						e.Obj3.datum.points = points; // FIXME bug ?
					}
				...
				case AssetType.PathTube:
					if (!e.Visual.material) {
						...
						if (e.Visual.shader) {
							...
							if (e.Visual.asset)
								// FIXME but AssetKeepr.loadTexure is asynchronous
								// tex = new THREE.TextureLoader().load( `assets/${e.Visual.asset}` );
								tex = AssetKeepr.loadTexure( e.Visual.asset );
							else // in case of test and default
								tex = new ramTexture(3, 4, {alpha: alp});
							var side = !e.Visual.paras || e.Visual.paras.side === undefined
										? THREE.DoubleSide : e.Visual.paras.side;
	...
	static xlineMaterial( uniforms = new Object(), vparas ) {
		...
		if (Array.isArray(vparas.color)) {
			uniforms.u_color = { value: new THREE.Vector3(...vparas.color) };
		}
		else if (vparas.color !== undefined) {
			var c = new THREE.Color(vparas.color);
			uniforms.u_color = { value: new THREE.Vector3(c.r, c.g, c.b) };
		}
		else
			uniforms.u_color = {value: new THREE.Vector3([1, 1, 0, 1]) };
		...
		if (vparas.dashSize !== undefined)
			matOpt.dashSize = vparas.dashSize;
		if (vparas.gapSize !== undefined)
			matOpt.gapSize = vparas.gapSize;
	...
	static createObj3Line( obj3, paras, material ) {
		...
		var line = new THREE.Line( geomCurve, material );
	...
	/**@deprecated LineBasicMaterial can only change color, not alpha - an issue
	 * discussed this but can't find again.
	 *
	...
	static createCurve(cmpObj3, paras) {
		...
		var matOpt = {transparent: true,
		    ...
		    varying vec2 vUv;
		    void main() {
		      // vec4 texel = vec4(u_alpha);
		      // gl_FragColor = opacity * u_alpha * texel;
			  gl_FragColor = vec4(u_color, u_alpha);
			  gl_FragColor.a = opacity * u_alpha;
		  }` };

		matOpt.uniforms = new Object();
		matOpt.uniforms.u_alpha = new Object();
		matOpt.uniforms.u_alpha.value = 1;
		matOpt.uniforms.u_color = {value: new THREE.Vector3(...(paras.color || [1, 1, 0, 1])) };
		matOpt.uniforms.opacity = new Object();
		matOpt.uniforms.opacity.value = 1;

		if (paras.linewidth !== undefined)
			matOpt.linewidth = paras.linewidth;
		if (paras.dashSize !== undefined)
			matOpt.dashSize = paras.dashSize;
		if (paras.gapSize !== undefined)
			matOpt.gapSize = paras.gapSize;

		// var linemat = new THREE.LineBasicMaterial( matOpt );
		var linemat = new THREE.ShaderMaterial( matOpt );

		var line = new THREE.Line( geomCurve, linemat );
		line.computeLineDistances();
	...
	static createDynaSects(cmpObj3, paras) {
		...
		for (var geom of geoms) {
			var mopt = { color: paras.color === undefined ? 0x0f7f8f : paras.color };
			if (paras.linewidth !== undefined)
				mopt.linewidth = paras.linewidth;
			var line = new THREE.Line( geom, new THREE.LineBasicMaterial( mopt ) );
			line.computeLineDistances();
			grp.add(line)
		}
	...
	static geometryCase(geomType, geomParaArr, vparas, obj3) {
		...
		if (geomType === Obj3Type.BOX) {
			...
			if (vparas && vparas.features) {
				var {geom, points} = AssetKeepr.geoHexacylinder( vparas.features, {
				...
				var {geom} = AssetKeepr.geoHexacylinderAsync(vparas.uri,
