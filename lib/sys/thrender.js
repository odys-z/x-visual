...
export default class Thrender extends ECS.System {
					...
					if (e.Visual.asset === null && e.Visual.options.model) {
						...
						AssetKeepr.loadGltfNodes(`assets/${e.Visual.asset}`,
							...
							(nodes) => {
								...
								var ms = Thrender.createGltfPoints(e.Obj3, nodes);
	...
	/**Create Obj3.mesh.
	 * This only handling Visual.vtype == AssetType.point
	 * @param {xv.Components.Obj3}
	 * @param {array} nodes nodes loaded from gltf.
	 */
	static createGltfPoints(cmpObj3, nodes) {
		if (cmpObj3 && nodes) {
			 ...
			 * nodes[0].children[0].geometry.attributes['position'] ==
				BufferAttribute {name: "", array: Float32Array(2772), itemSize: 3, count: 924, normalized: false, …}
				length: (...)
				dynamic: (...)
				name: ""
