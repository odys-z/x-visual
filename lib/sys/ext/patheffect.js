		...
		if (flowings.size > 0) {
			...
			for (var e of flowings) {
				if (e.Occluder && e.Occluder.FlowingPath)
					e.Obj3.occluding |= LayerFilter.FLOWING_PATH ;
				else // FIXME move to Orthocclude?
					e.Obj3.layers |= LayerFilter.FLOWING_PATH ;
		...
		if (filmings.size > 0) {
			...
			for (var e of flowings) {
				if (e.Occluder && e.Occluder.Filming)
					e.Obj3.occluding |= LayerFilter.FILMING;
				else
					e.Obj3.layers |= LayerFilter.FILMING;
