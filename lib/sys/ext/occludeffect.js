...
import OrthoEffect from './orthoeffect'

/**
 * @class
 */
export default class Occludeffect extends OrthoEffect {
	constructor(ecs, x) {
		super(ecs, x);
		this.ecs = ecs;
		...
		if (x.options.background instanceof THREE.Material)
			...
			console.warn(`Before ${x.ver} Occludeffect background can only support parameter of Material.`);
...
Occludeffect.query = {any: ['FlowingPath', 'GlowingEdge', 'Filming']};
