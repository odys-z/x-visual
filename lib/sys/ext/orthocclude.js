...
export default class Orthocclude extends XSys {
	constructor(ecs, x) {
		...
		if (effects && effects.length > 0) {
			...
			if (this.passLayers)
				this.passLayers.unify(layers !== undefined ? layers : LayerChannel.ALL);
			else this.passLayers = layers !== undefined ? layers : LayerChannel.ALL;
		}
	...
	update (tick, entities) {
		...
		for (var e of entities) {
			if ( this.passLayers.occlude( e.Obj3.occluding )) {
				...
				// FIXME yes, postprocessing is a heavy computation
				Orthocclude.pushLayer(e.Obj3.mesh, e.Obj3.layers);
		...
		for (var e of this.darkens) {
			...
			// FIXME yes, postprocessing is a heavy computation
			Orthocclude.popLayer(e.Obj3.mesh);
		}
		...
		if (mesh.children && mesh.children.length > 0)
			for (const c of mesh.children) {
				if (c.layers)
					...
					Orthocclude.pushLayer(c.mesh, l);
			}
	}

	static popLayer(mesh) {
		mesh.layers.pop(l);
		if (mesh.children && mesh.children.length > 0)
			for (const c of mesh.children) {
				if (c.layers)
