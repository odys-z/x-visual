...
export default class Htmltex extends XObj {
	constructor(ecs, options) {
		...
		if (options.entDefines) {
			for (var d of options.entDefines) {
				this.canvas[d.Canvas.domId] = d.Canvas;
			}
		}
		else {
	...
	refresh(htmlId, onRefresh) {
		...
		// 	AssetKeepr.loadCanvtex(this.entitys[htmlId].Canvas);
		// }
		// else {
		// 	const ents = Object.values(this.entitys);
		// 	if (ents && ents.length > 0) {
		// 		ents.forEach(function(e, ix) {
		// 			AssetKeepr.loadCanvtex(e.Canvas, this.tick);
		// 		});
		// 	}
		// }
		AssetKeepr.loadCanvtex(this.canvas[htmlId], undefined, onRefresh);
	}
