...
export default class CanvTex extends XSys {
	...
	init(entities) {
		...
		function createTextPlane( obj3, visual, dynatex ) {
			...
			var uniforms = xglsl.formatUniforms(visual, obj3);
			// var uniforms = obj3.mesh && obj3.mesh.material ?
			// 				obj3.mesh.material.uniforms :
			// 				xglsl.formatUniforms(visual, obj3);
		...
		for (const e of entities) {
			if ( e.Dynatex && e.Dynatex.dirty ) {
				if ( !e.Dynatex.textplane )
					...
					var tex = AssetKeepr.drawText2(e.Dynatex);
					//console.log(tex);
