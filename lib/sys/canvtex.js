	...
	init(entities) {
		...
		for (const e of entities) {
			...
			if (e.Visual.vtype === AssetType.Extension) {
				...
				e.Visual.paras = Object.assign(e.Visual.paras || {}, vparas);
			}
		...
		function createTextPlane( obj3, visual, dynatex ) {
			...
			u_tex.push( 'data:application/x-visual+img,grey-pixel' );
			...
			var h = dynatex.xywh.size + (dynatex.xywh.margin || 0) * 2;
		...
		for (const e of entities) {
			if ( e.Dynatex && e.Dynatex.dirty ) {
				if ( !e.Dynatex.textplane )
					...
					console.error('Dyantex.texplane is not ready?', e);
				else {
