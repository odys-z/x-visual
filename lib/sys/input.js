...
export default class Input extends ECS.System {
	...
	constructor(ecs, x) {
		...
		var picker = x.picker;
		if (picker === undefined) {
			x.gpuPicker = new GpuPicker(x.renderer);
			picker = this.gpuPicker(x);
		}
	...
	gpuPicker(x) {
		...
		const scene = x.scene;
		const picker = x.gpuPicker;

		return function (code, e) {
			// var cmd = {code, 'button': evt.button, e: evt};
			// return cmd;
			if (code === 'mouse') {
				picker.pickTest([e.clientX, e.clientY], scene, camera, 0.5);
				var cmd = {code, 'cmd': picker.pickedId, e};
				return cmd;
			}
		};
	}
