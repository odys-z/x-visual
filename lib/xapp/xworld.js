...
export default class XWorld {
	...
	constructor(canvas, wind, opts) {
		...
		canvas.appendChild( this.saveLink );
	...
	// 		binary: opt.binary === true,
	// 		maxTextureSize: opt.maxTextureSize || 1024 * 64
	// 	};
	// 	gltfExporter.parse( this.xscene, onsave );
	// }

	xport(opt = {}) {
		let reglb = /\.gltf\s*$/i;
		let filename = opt.filename;
		let binary = !reglb.test(filename);

		if (!filename){
			if (binary)
				filename = "xscene.glb";
			else filename = "xscene.gltf";
		}
		this.saveLink.download = filename;
		let link = this.saveLink;
		...
		gltfExporter.parse( this.xscene, function(result) {
