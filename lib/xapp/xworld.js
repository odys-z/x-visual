...
import GlowingEffect from '../sys/ext/glowingeffect'
...
export default class XWorld {
	...
	constructor(canvas, wind, opts) {
		...
		ecs.componentTriggered (['PathEffect', 'Glow', 'Filming'],
			(def) => x.options.effects = true);
		x.ecs = ecs;
	...
	startUpdate() {
		...
		if (x.options.pathEffect) {
			var occludeEffect = new PathEffect(ecs, x);
			ecs.addSystem('render', occludeEffect);
		}
