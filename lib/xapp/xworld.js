...
export default class XWorld {
	...
	startUpdate() {
		...
		// dependency:
		// PathEffect <- FinalComposer (options.pathEffects == true)
		// Thrender <- PathEffect (options.pathEffects == true)
		// Thrender <- Inputs
		// Thrender <- Animizer
		// XTweener <- Animizer
		var ecs = x.ecs;
		...
		if (x.options.pathEffects) { // FIXME -> x.effects.path
			var occludePass = new PathEffect(ecs, x);
