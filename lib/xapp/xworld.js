...
import {TweenAnim} from '../sys/anims' // TODO deprecate
...
export default class XWorld {
	...
	registerComponents(ecs, comps) {
		if (comps) {
			if (x.log >= 5) console.log('[log 5] register components ', comps);
	...
	startUpdate() {
		...
		if (!x.options.tween) {
			// TODO make it clearer: animizer, not same as animator
			ecs.addSystem('tween', new MorphingAnim(ecs, {}));
			ecs.addSystem('tween', new XTweener(ecs, x));
