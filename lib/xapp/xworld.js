...
export default class XWorld {
	constructor(canvas, wind, options) {
		...
		this.registerComponents(ecs, {Visual, Obj3});
	...
	startUpdate() {
		...
		// add subsystems
		ecs.addSystem('mvc', new Input(ecs, x.window));
		...
		if (x.options.picker === 'gpu' || (x.options.picker && x.options.picker.gpu)) {
			ecs.addSystem('mvc', new GpuPicker(ecs, {}));
		}
