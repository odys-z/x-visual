...
import Occludeffect from '../sys/ext/occludeffect'
...
export default class XWorld {
	...
	startUpdate() {
		...
		// dependency:
		// Occludeffect <- FinalComposer (options.postEffects == true)
		// Thrender <- Occludeffect (options.postEffects == true)
		// Thrender <- Inputs
		// Thrender <- Animizer
		// XTweener <- Animizer
		var ecs = x.ecs;
		...
		if (x.options.postEffects) {
			var occludePass = new Occludeffect(ecs, x);
