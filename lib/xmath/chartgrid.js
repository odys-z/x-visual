...
export class CoordsGrid {
	 ...
	 * @param {array} idx grid index, [ x, y = 0, z], if y ignored, return [x, 0, z] in world
	 * @param {...array} offset ...[x, y, z], offset in grid
	 * @return {object} position (buf)
	 * @member CoordsGrid#worldPos
	 * @function
	 */
	worldPos(buf, idx, ...offset) {
		var p = buf || [0, 0, 0];
		// p[0] = idx[0] * this.scale[0];
		// p[1] = idx[1] * this.scale[1];
		// p[2] = idx.length > 2 ? idx[2] * this.scale[2] : 0;
		// return p;
		if (idx.length > 2) {
			p[0] = idx[0]; // * this.scale[0];
			p[1] = idx[1]; // * this.scale[1];
			p[2] = idx[2]; // * this.scale[2];
		}
		else {
			p[0] = idx[0]; // * this.scale[0];
			p[1] = 0;
			p[2] = idx[1]; // * this.scale[2];
		}
		if (offset) {
	 ...
	 * @param {array} cxyz, [x, y, z] section count
	 * @return {array} array of positions, in x, y, z
