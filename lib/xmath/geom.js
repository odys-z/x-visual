...
export default class geom {
	...
	static texPrism3857(cell, geoCentre, ctx) {
		...
		for (let ix = 1; ix <= poly; ix++) {
			...
			ctx.ix += setLateralFace(v0, v1, n, h, ctx.vx, ix - 1);
		}
		...
		function setLateralFace(v0, v1, n, h, vx0, side) {
			function setLateralNormal(norms, norm, ix) {
				if (vec3.len(norm) > 0)
					norms.set(vec3.unit([norm[0], 0, norm[2]], nbuff), ix*3);
				else norms.set([0, 0, 0]);
			}
			// a    d  (upper follows)
			// |    |   ...
			// b    c  (lower first)
		...
		/**Set 1 roof face's vetex, includeing pos, box, atile, loc, uv, normal.
		 * @param {array} v0 geo ploygon offset
		 * @param {float} h height
		 * @param {array} geoxzwh geo-polygon x, z, w, h
		 * @param {int} jx vertex index
		 */
