...
export default class geom {
	 ...
	 */
	static generateWayxz(path, y0) {
		const halfW = 10;
		// for face, use plane XZ (y = 0)
		// pathxz = new Array(path.length * 2);
		...
		var dirs = new Float32Array(plen * 3);
		var waypoints = new Float32Array(2 * plen * 3 * 3); // double side, 3 attributes
		for (var pi = 0; pi < plen; pi++) {
			var point = [];

			if (pi < plen - 1)
				dir = pathDir(path, pi, dir);
			// else use the last dir
			dirs.set(dir, pi * 3);

			var p0 = [path[pi][0], 0, path[pi][1]];    // xz plane of geo point

			vec3.setLength(halfW, dir);
			vec3.crossArr(dir, [0, -1, 0], l0);
			l0 = vec3.addArr(p0, l0, l0);
			l0[1] = y0;

			vec3.crossArr(dir, [0, 1, 0], r2);
			r2 = vec3.addArr(p0, r2, r2);
			r2[1] = y0;
			...
			if (pi > 1) {
				...
				// l1: 2len - 1 - (i + 1)
				// l2: 2len - 1 - (i + 2)
				var l1 =  [ waypoints[(plen * 2 - pi+2) * 3],
							waypoints[(plen * 2 - pi+2) * 3 + 1],
							waypoints[(plen * 2 - pi+2) * 3 + 2] ];
				var l2 =  [ waypoints[(plen * 2 - pi+3) * 3],
							waypoints[(plen * 2 - pi+3) * 3 + 1],
							waypoints[(plen * 2 - pi+3) * 3 + 2] ];
				l1 = checkSide(l0, l1, l2);
				waypoints.set(l1, (plen * 2 - pi + 1) * 3);

				var r1 =  [ waypoints[(pi-1) * 3],
							waypoints[(pi-1) * 3 + 1],
							waypoints[(pi-1) * 3 + 2] ];
				var r0 =  [ waypoints[(pi-2) * 3],
							waypoints[(pi-2) * 3 + 1],
							waypoints[(pi-2) * 3 + 2] ];
				r1 = checkSide(r0, r1, r2);
				waypoints.set(r1, (pi-1) * 3);
			}

			waypoints.set(l0, (plen * 2 - pi + 1) * 3);
			waypoints.set(r2, pi * 3);
		}
		// TODO add round cap?

		var pathGeometry = new THREE.BufferGeometry();
		pathGeometry.addAttribute("position", new THREE.BufferAttribute(waypoints, 3));
		pathGeometry.addAttribute("geoLoc", new THREE.Float32BufferAttribute(path, 3));
		pathGeometry.addAttribute("geoDir", new THREE.Float32BufferAttribute(dirs, 3));
		return pathGeometry;

		function pathDir(geopath, ix, buff) {
			// vec3.subArr(path[ix + 1], path[ix], buff)
			return vec3.subArr(
				[geopath[ix+1][0], 0, geopath[ix+1][1]],
				[geopath[ix][0], 0, geopath[ix][1]], buff);
		}

		function checkSide(p0, p1, p2) {
