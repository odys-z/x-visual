...
export default class geom {
	...
	static generateWayxz(funcode, path, y0) {
		const halfW = 10;
		...
		var waypoints = new Float32Array(plen * 3 * 2);
		for (var pi = 0; pi < plen; pi++) {
			...
			if (pi > 1) {
				...
				var l1 =  [ waypoints[plen * 2 - (pi+1) * 3],
							waypoints[plen * 2 - (pi+1) * 3 + 1,
							waypoints[plen * 2 - (pi+1) * 3 + 2 ];
				var l2 =  [ waypoints[plen * 2 - (pi+2) * 3,
							waypoints[plen * 2 - (pi+2) * 3 + 1,
							waypoints[plen * 2 - (pi+2) * 3 + 2 ];
				l1 = geom.checkSide(l0, l1, l2);
		...
		function checkSide(p0, p1, p2) {
			var reslt = checkLineIntersection(
				p0[0], p0[2], p1[0], p1[2],
				p1[0], p1[2], p2[0], p2[2] );
