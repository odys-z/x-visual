	...
	static generateWayxz(path, y0) {
		...
		for (var pi = 0; pi < plen; pi++) {
			...
			if (pi < plen - 1)
				...
				vec3.subArr(path[pi+1], path[pi], dir);
			// else use the last dir
			dirs.set(dir, pi * 3);

			var p0 = [path[pi][0], 0, path[pi][2]];    // xz plane

			vec3.crossArr(dir, [0, -1, 0], l0);
			vec3.setLength(halfW, l0);
			l0 = vec3.add(path[pi], p0, l0);
			l0[1] = y0;

			vec3.crossArr(dir, [0, 1, 0], r2);
			vec3.setLength(halfW, r2);
			r2 = vec3.add(path[pi], p0, r2);
			r2[1] = y0;
