...
export default class geom {
	 ...
	 * Generated road model is applied with style.geoScale, default 1. So if the
	 ...
	 * @param {object} style {halfWidth: number, geoScale: number}
	...
	/**Generate a tile of hexagon volume. All coordinates are in world.
	 * Hexagon center = (cell - geoCentre) * scale, with height ignoring scale.
	 * And world z is reversed, z = - (cell.y - geoCentre.y)
	 *
	 * This function is supposed to be called by a geojson stream handler.
	 *
	 * Reference:<br>
	 ...
	 * Best way of sharing vertices: flat</a>
	 ...
	 * Three.js cylinder way: CylinderGeometry#generateCap()</a>
	 * <a href='http://www.opengl-tutorial.org/intermediate-tutorials/tutorial-9-vbo-indexing/'>
	 * reading: OpenGl Tutorial - VBO Indexing</a>
	 * @param {object} cell  information of cell geomerty to be created
	 * cell.coord: 2d position, in epsg 3857
	 * cell.height: default 1
	 * cell.geoscale, default 1
	 ...
	 * @member xgeom.hexacylinder3857
	 * @function
	 */
	static hexacylinder3857(cell, geoCentre, tile0) {
		var c0 = [0, 0, 0];
		vec3.subArr(cell.coord, geoCentre, c0);
		vec3.mulArr(c0, cell.geoScale || 1, c0);
		var vbuff = [0, 0, 0];
		const h = cell.height;
		...
		var uvs = new Float32Array(verts * 2);
		var normals = new Float32Array(verts * 3);
		var dirs = new Float32Array(verts * 3);
		var index = [];
		...
		// end caps
		var fx0 = ix; // starting cap vertex' index
		// 6 verts down, 6 verts up
		for (v of tile0) {
			setCapverts(v, c0, h, ix);
			ix++ ;
		}

		/* TODO doc the end faces
		      3
			  ^
		  4  /_\ 2
		   |\  /|
		...
		*/
		...
		// hex.setAttribute("position", new THREE.BufferAttribute(pos, 3));
		// hex.setAttribute("a_h", new THREE.BufferAttribute(h, 3));
		// hex.setAttribute("a_tan", new THREE.BufferAttribute(dirs, 3));
		// hex.setAttribute( 'normal', new THREE.BufferAttribute( normals, 3 ) );
		// hex.setAttribute( 'uv', new THREE.BufferAttribute( uvs, 2 ) );
		// hex.setIndex(index);

		return {pos, uvs, normals, dirs, index};

		// set a cylinder's up-down verts
		function setColumn(v, c0, h, ix) {
			vec3.addArr(v, c0, vbuff);
			pos.set(vbuff, ix * 3);
			dirs.set(v, ix * 3);
			uvs.set([ix/6, 0], ix * 2);
			normals.set([ix, 0, 0], ix * 3); // TODO let's learn from Three.js

			const jx = ix + stride;
			pos.set([vbuff[0], vbuff[1] + h, vbuff[2]], jx * 3);
			dirs.set(v, jx * 3);
			uvs.set([ix/6, 1], jx * 2);
			normals.set([ix, h, 1], jx * 3);
		}

		// generate all 12 vertices of cap faces
		function setCapverts(v, c0, h, ix) {
			// lower
			vec3.addArr(v, c0, vbuff)
			pos.set(vbuff, ix * 3);
			dirs.set(v, ix * 3);
			uvs.set([v[0], v[2]], ix * 2);
			normals.set([0, -1, 0], ix * 3);

			// upper
			const jx = ix + stride;  // stride from lower vertex to the upper
			pos.set([vbuff[0], vbuff[1] + h, vbuff[2]], jx * 3);
			dirs.set(v, jx * 3);
			uvs.set([v[0], v[2]], jx * 2);
			normals.set([0, 1, 0], jx * 3);
		}

		// create 2 faces of tile0[a, b, c], at c0
		function capFace(a, b, c, h, f0) {
			// down side
			index.push(a + f0, c + f0, b + fx);
			// up
			index.push(a + f0 + stride, b + f0 + stride, c + f0 + stride);
		}
	}

	static hexacylinder3858(cell, geoCentre, tile0, ctx) {
		var c0 = [0, 0, 0];
		vec3.subArr([cell.coord[0], 0, cell.coord[1]], [geoCentre[0], 0, geoCentre[1]], c0);
		vec3.mulArr(c0, cell.geoScale || 1, c0);
		var vbuff = [0, 0, 0];
		const h = cell.height;
		...
		// var dirs = new Float32Array(verts * 3);
		// var index = [];
		var {pos, uvs, normals, dirs, index} = ctx;
		...
		*/
		...
		// set a cylinder's up-down verts
		function setColumn(v, n, c0, h, ix) {
			vec3.addArr(v, c0, vbuff);
			pos.set(vbuff, ix * 3);
			dirs.set(v, ix * 3);
			uvs.set([ix/6, 0], ix * 2);
			normals.set([n[0], 0, n[2]], ix * 3);

			const jx = ix + stride;
			pos.set([vbuff[0], vbuff[1] + h, vbuff[2]], jx * 3);
			dirs.set(v, jx * 3);
			uvs.set([ix/6, 1], jx * 2);
			normals.set([n[0], h, n[2]], jx * 3);
		}
	...
	static hexatile(r) {
		r = r || 1;
		// √3/2 * r, 0.5 * r
		const r32 = Math.sqrt(3) * 0.5 * r;
		const r05 = r * 0.5;
		return {vert: [[  0, 0,  -r ], [r32, 0, -r05], [r32, 0, r05], [ 0,   0,  r ], [-r32, 0, r05], [-r32, 0, -r05]],
				norm: [[r05, 0, -r32], [  r, 0,   0 ], [r05, 0, r32], [-r05, 0, r32], [ -r,  0,  0 ], [-r05, 0, -r32]]};
	}
}
