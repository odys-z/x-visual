...
export function ramTexture(w, h, alpha) {
    ...
    for ( var i = 0; i < size; i ++ ) {
    	...
    	data[ stride ] = ((i - 255) / size * 255) % 255;
    	data[ stride + 1 ] = (i / size * 255) % 255;
    	data[ stride + 2 ] = ((i + 255) / size * 255) % 255;
        data[ stride + 3 ] = a;
    }
...
export function vec3(xyz) {
    throw XError('xcommon.vec3() is replaced by xv.packages.vec.vec3.js()');
