...
function phongMorph2(vparas) {
	...
	if (!colr || tex && !Array.isArray(tex)
		...
		|| colr.length <= 0)
		throw new XError(`paras.u_tex and paras.colors not matching (at least on color): ${vparas}`);

	var len = colr.length;
	var colori = `uniform vec3 u_color[${len}]; uniform sampler2D u_tex[${len}];`;
	var morphv =
       `vec3 morph = u_color[0];
        for (int i = 0; i < ${len}; i++) {
            morph = mix(morph, u_color${i}, u_morph${i - 1});
		}
		return morph;`;
