 ...
 * @param {number} t 0 - 1
 * @param {THREE.Vector3} [tweenBuffer]
 * @member xglsl.pathPoint
export function pathPoints(path, t, tweenBuffer) {
	if (path instanceof THREE.Curve) {
		return path.getPointAt(t, tweenBuffer);
	}
	else throw new XError("xglsl.pathPoints(): Only path of THREE.Curve can be used to generate point.");
}
 */
