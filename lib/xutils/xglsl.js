...
export function formatUniforms(cmpVisual) {
	var uniforms = {};
...
function cubeVoxels1(paras) {
 return { vertexShader: `
  ...
  vec4 ix2model(vec3 sects, vec3 box) {
    return vec4((position - sects / 2.0) * box / sects, 1.);
  }

  void main() {
	vec4 pos = ix2model(u_sects, u_box0);
	pos = mix(pos, ix2model(u_sects, u_box1), u_morph);
...
function cubeVoxels2(paras) {
 ...
 return { vertexShader: `
  ...
  void main() {
	...
	${pos}
    gl_Position = projectionMatrix * modelViewMatrix * pos;
	gl_PointSize = 3.0 * ${paras.vert_scale !== undefined ? paras.vert_scale : '3'};
  }
  `,
