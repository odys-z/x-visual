 ...
 return {
  fragmentShader: `
    ...
    vec4 mainImage( in vec2 fragCoord ) {
      ...
      for (int i = 0; i < ${orbs}; i++) {
        vec3 ci = vcent[i].xyz;
        float ri = r[i];
        ri *= sin(now * speed * 0.6 + float(i) * 0.3) + 1.2;
        ...
        fragColor += pow( clamp(ratio, 0.0, 1.), 8. + (abs(sin(now * speed + float(i) * 0.06)) * 4.) ) * orbColors[i];
        fragColor.a = 1.2 - ri;
      }
      fragColor = clamp(fragColor, 0., 1.);
      float txAlpha = clamp(whiteAlpha, 0., 1.);
  ...
  vertexShader: `
    ...
    void main() {
      ...
      vscale = orbScale;
      for (int i = 0; i < ${orbs}; i++) {
        vcent[i] = modelMatrix * vec4(a_loc + wpos + offsets[i] * 2. * sin(now * speed * 0.1 + float(i) * 0.3), 1.);
      }
...
/**
 * orb-center = a_pos + u_tilePos + offset
 * uniform vec3 wpos: position offsets in world - should be in tile range for tile rendering
 * attribute vec3 a_loc: such as tile center in world.
 * @param {object} vparas paras.vert_scale [optional] number scale of vertices
 * @return {object} {vertexShader, fragmentShader}
 * @member xglsl.tiledOrbs
 * @function
 */
function tiledOrbs(vparas) {
 var orbs = vparas.offsets.length || 1;
 return {
  fragmentShader: `
    uniform float r[${orbs}];
  ...
  vertexShader: `
    ...
    void main() {
      ...
      vscale = orbScale;
      for (int i = 0; i < ${orbs}; i++) {
        vcent[i] = modelMatrix * vec4(a_loc + wpos + offsets[i], 1.);
      }
