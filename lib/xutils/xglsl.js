...
function cubVoxels (paras) {
 return { vertexShader: `
  uniform float u_alpha;
  uniform vec3 u_sects;
  uniform vec3 u_box0;
  uniform vec3 u_box1; uniform float u_morph; niform mat4 u_trans1;
  ...
  void main() {
    vColor = color;
	vAlpha = u_alpha;
	vec4 pos0 = ix2world(position, a_box0);
	vec4 pos1 = ix2world(position, a_box1);
	pos0 = mix(modelViewMatrix * pos0, u_trans1 * pos1, u_morph);
  ...
  vec3 ix2world(sects, box) {
    return vec3(box * sects / u_sects, 1.0);
  }`,
