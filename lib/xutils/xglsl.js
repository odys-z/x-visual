...
import {texPrism, boxLayers, boxRotate} from './shaders/shape-layers.glsl'
...
export function formatUniforms(visual, obj3) {
	...
	if (visual.shader === ShaderFlag.colorArray && visual.paras && visual.paras.colors) {
		...
		if (visual.paras.tile && visual.paras.tile.hue) {
			uniforms.u_hue = { value: visual.paras.tile.hue };
		}
		else
			uniforms.u_hue = { value: [0,8, 0.8, 0.8, 0.8] };

		if (visual.paras.tile.north !== undefined)
