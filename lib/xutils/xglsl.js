...
export function formatUniforms(visual, obj3) {
    ...
    if (visual.paras && visual.paras.u_tex) {
        ...
        uniforms = {
            u_tex: { value: new THREE.TextureLoader().load(`assets/${visual.paras.u_tex}`) }
        };
    }
    else if (visual.paras.uniforms && typeof visual.paras.uniforms.u_tex === 'string') {
        ...
        //                 visual.paras.uniforms.u_tex );
        uniforms = {
            u_tex: { value: new THREE.TextureLoader().load(`assets/${visual.paras.uniforms.u_tex}`) }
        };
        delete visual.paras.uniforms.u_tex;
    }
    ...
    if (visual.shader === ShaderFlag.scaleOrb) {
	...
	else if (visual.shader === ShaderFlag.colorArray) {
        ...
        var poses = [];
        var orbs = 0;
        for (var p of visual.paras.wpos || [[0, 0, 0]]) {
...
function worldOrbs(vparas) {
 ...
 return { fragmentShader: `
  ...
  void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    // texcolor.a = clamp(whiteAlpha, 0.0, 1.);
    // fragColor = mix(fragColor, texcolor, clamp(whiteAlpha, 0.0, 1.));
  ...
  void main() {
    ...
    vscale = orbScale;
    for (int i = 0; i < ${orbs}; i++){
      cent[i] = modelMatrix * vec4(wpos[i], 1.);
    }
