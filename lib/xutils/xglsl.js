 ...
 * uniform vec3 u_tilePos: position offsets in world - should be in tile range for tile rendering
...
function tiledOrbs(vparas) {
 ...
 vertexShader: `
  ...
  void main() {
    ...
    vscale = orbScale;
    for (int i = 0; i < ${orbs}; i++){
      vcent[i] = modelMatrix * vec4(a_loc + u_tilePos + offsets[i], 1.);
    }
