...
export function formatUniforms(visual, obj3) {
	...
	// u_color
	if (visual.paras.color) {
		uniforms.u_color = {value: visual.paras.color};
	}
	...
	if (visual.shader === ShaderFlag.colorArray && visual.paras && visual.paras.colors) {
		// TODO should we use uniform array ?
		for (var i = 0; i < visual.paras.colors.length; i++)
			uniforms[`u_color${i}`] = {value: new THREE.Vector4(...visual.paras.colors[i])};
	}
		...
		if (visual.paras.yztile)
			wpos.x += visual.paras.yztile.x0 || 0;
		if (visual.paras.xztile)
			wpos.y += visual.paras.xztile.y0 || 0;
		if (visual.paras.xytile)
			wpos.z += visual.paras.xytile.z0 || 0;
		uniforms.wpos = { value: wpos };
		...
		if (visual.paras.tile && visual.paras.tile.floorHeight !== undefined) {
			uniforms.floorHeight = { value: visual.paras.tile.floorHeight };
		}
		...
		if (visual.paras.tile && visual.paras.tile.hue) {
			...
			uniforms.u_hue = { value: new THREE.Vector4(0,8, 0.8, 0.8, 0.8) };

		if (visual.paras.tile && visual.paras.tile.north !== undefined)
			uniforms.u_north = { value: visual.paras.tile.north };
		else
			uniforms.u_north = { value: Math.PI / 4 };
	}
