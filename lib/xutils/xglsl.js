...
import {texPrism, boxLayers} from './shaders/shape-layers.glsl'
...
export function formatUniforms(visual, obj3) {
	...
	// common section
	// u_tex
	if (visual.paras.uniforms && typeof visual.paras.uniforms.u_tex) {
		...
		Visual.paras = Object.assign(Visua.paras, {u_tex: visual.paras.uniforms.u_tex});
	...
	if (visual.shader === ShaderFlag.colorArray && visual.paras && visual.paras.colors) {
		...
		if (visual.paras.floorHeight) {
			uniforms.floorHeight = { value: visual.paras.floorHeight };
		}
	}
