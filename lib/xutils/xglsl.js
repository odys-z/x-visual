...
export function formatUniforms(visual, obj3) {
    ...
    if (visual.shader === ShaderFlag.colorArray && visual.paras.colors) {
        ...
        var poses = [];
        var orbs = 0;
        for (var p of visual.paras.offsets || [[0, 0, 0]]) {
...
function orbGroups(vparas) {
 ...
 return { fragmentShader: `
  ...
  varying vec2 vUv;
  varying vec3 P;
  varying vec3 vtan; // ignored in 0.3
  varying vec3 vscale;
 ...
 */
 vertexShader: `
  ...
  void main() {
    ...
    // gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );
    vec4 worldPosition = modelMatrix * vec4(position, 1.0);
    // vec4 worldTan = modelMatrix * vec4( a_tan, 1.0 );
    ...
    for (int j = 0; j < ${grps}; j++) {
      for (int i = 0; i < ${orbs}; i++) {
        ...
        // vec3 vc = (modelMatrix * vec4(wpos[j], 1.)).xyz;
        // vcent[j*${grps} + i] = vc; // + normalize(a_tan) * offsets[i];


        // vec4 vc = modelMatrix * vec4(wpos[j] + normalize(a_tan) * offsets[i], 1.);
        vec4 vc = modelMatrix * vec4(wpos[j] + normalize(a_tan) * 10. * float(i), 1.);
        vcent[j*${grps} + i] = vc.xyz;
      }
    }
