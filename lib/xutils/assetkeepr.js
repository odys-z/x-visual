...
export default class AssetKeepr {
	...
	/**@property {THREE.Container} canvas */
	static get canvs() { return assets.canvs; }
	/**@property {object} assets - global assets */
	static get assets() { return assets; }
	/**@property {number} log - log level */
	...
	static loadCanvtex2(cmpCanv, options, onUpdate) {
		var opts = Object.assign({width: 256, height: 256},
								 options);
		html2canvas(cmpCanv, opts).then( function(canvas) {
			var dom = document.getElementById(cmpCanv.domId);
			var drawingContext = canvas.getContext( '2d' );
			...
			cmpCanv.tex = new THREE.CanvasTexture(drawingContext.canvas);
			cmpCanv.ctx2d = drawingContext;
			// dirty is used diferently to Htmltex in D3 charts
			// cmpCanv.dirty = false; // text is updated to svg
