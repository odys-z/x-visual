...
import { GLTFExporter } from '../../packages/three/GLTFExporter'
...
export default class AssetKeepr {
	...
	static geoPrismAsync(url, options, onload,
		...
		var verts = options.maxVerts || 4096 * (6*4 + 6); // 4k hexagon (FIXME)
		// context
		var ctx = new Object();
			ctx.features = 0;
			ctx.vx = 0; // starting vert index for each feature.
			ctx.ix = 0; // index' index
			ctx.pos = new Float32Array(verts * 3);
			ctx.loc = new Float32Array(verts * 3);
			ctx.atiles = new Float32Array(verts * 3);
			ctx.uvs = new Float32Array(verts * 2);
		...
		.done((args) => {
			...
			geom.setAttribute('position', new THREE.BufferAttribute(ctx.pos.slice(0, ctx.vx * 3), 3));
			geom.setAttribute('a_box', new THREE.BufferAttribute(ctx.box.slice(0, ctx.vx * 3), 3));// box.y = groupidx
			geom.setAttribute('a_tiles', new THREE.BufferAttribute(ctx.atiles.slice(0, ctx.vx * 3), 3));
			geom.setAttribute('a_loc', new THREE.BufferAttribute(ctx.loc.slice(0, ctx.vx * 3), 3));
			geom.setAttribute('normal', new THREE.BufferAttribute(ctx.normals.slice(0, ctx.vx * 3), 3));
			geom.setAttribute('uv', new THREE.BufferAttribute(ctx.uvs.slice(0, ctx.vx * 2), 2));
			geom.setIndex(ctx.index.slice(0, ctx.ix));
	...
	static exportGltf(scene, filename, options) {
