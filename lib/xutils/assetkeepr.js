...
export default class AssetKeepr {
	...
	static geojsonPaths(url, options, onLoad,
		...
		]; // stub for creating mesh / path line
	}
	...
	static geoTexturePrism(features, options) {
		...
		var heightName = options.heightName || 'height';
		var verts = (features.length || 0) * (6 * 4 + 6); // only upword faces, FIXME supposing hexagon only
		var ctx = new Object();
		...
		for(var f of features) {
			...
			// var xzwh = {x: -200, y: -200, w: 400, h: 400}; // TODO roof boundings
			var boxSize = options.box || [xzwh.w, 30, xzwh.h];

			var tile = options.tiles ||
						[4, options.tile ? potions.tile.layers || 1 : 1, 3];

			if (typeof options.beforeFeature === 'function')
				options.beforeFeature(f);
	...
	static geoPrismAsync(url, options, onload,
		...
		var heightName = options.heightName || 'height';
		var verts = options.verts || 1024 * (6*4 + 6); // 1k hexagon (FIXME)
		var ctx = new Object();
		ctx.features = 0;
		ctx.vx = 0; // starting vert index for each feature.
		ctx.pos = new Float32Array(verts * 3);
		...
		return { geom: new THREE.PlaneBufferGeometry(100, 100, 0) };
	}
