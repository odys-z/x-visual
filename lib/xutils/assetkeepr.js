...
export default class AssetKeepr {
	...
	static geoTexturePrism(features, options) {
		...
		for(var f of features) {
			...
			var boxSize = [120, 20, 75]; // TODO boxSize.y = layer distance
			// var xzwh = {x: -200, y: -200, w: 400, h: 400}; // TODO roof boundings
			var boxSize = options.box || [xzwh.w, 30, xzwh.h];

			var tile = Object.assign(options.tile || {},
							{tiles: [4, options.tile ? potions.tile.layers || 1 : 1, 3]} );

			xgeom.texPrism3857( {
				points,
				boxSize, xzwh, tile: tile.tiles,
				height: h * hs,
				prismCentre: loc,
				geoScale: options.geoScale || 1,
				group: options.onGroup ? options.onGroup(fx, f) : 1
