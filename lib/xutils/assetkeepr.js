...
export default class AssetKeepr {
	...
	/**
	 * 
	 * suggest scale plane to adapt more words and less words 
	 * this is only scale texture;
	 * 
	 */
	static drawText2(cmp) {
		...
		if(!assets.dynatex[text]) {
			...
			if (xywh.w > 0 && width > 0 && width > xywh.w){
				...
				hscale *= width/xywh.w;
				ctx.canvas.width = xywh.w*hscale;
				ctx.canvas.height = xywh.h*hscale;
			}
			else{
				
			ctx.canvas.width = xywh.w;
			ctx.canvas.height = xywh.h;
			}
			// debug notes: it's critical to set font after setting size
			ctx.font = fnt;
			ctx.textBaseline = 'top';
