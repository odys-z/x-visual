...
export default class AssetKeepr {
	...
	static cheapixelTex() {
		return AssetKeepr.loadTexure('data:application/x-visual+img,grey-pixel');
	}
	...
	static loadTexure(url, onload) {
		...
		if (!url)
			...
			if ( url.substr(30, 11) // it's 10, use 11 for safe
					=== 'grey-pixel' )
	 ...
	 * @member AssetKeepr.geojsonPaths
	 * @function */
	static geoHexacylinder(url, options, onload,
			onError = (args) => {console.error(args);} ) {
		var op = new Object();
		op.heightKey = 'height';
		op.cells = new Array();
		...
		op.radius = 1;
		op = Object.assign(op, options);
		op.tile0 = xgeom.hexatile(op.radius);

		var hex = new THREE.BufferGeometry();
		oboe( url )
			.node ( 'features.*', (feature) => {
				var h = feature.property[op.heightKey] || 1;
				var elem = feature.geometry.coordinates;
				var tuple = xgeom.hexacylinder3857(
							{ coord: elem, height: op.height, geoScale: op.scale },
							op.center, op.tile0);
				op.cells.push(tuple);
				opt.wpos.push(tuple.pos);
			} )
			.done ( (args) => {
				// console.log(args);
				var vlen = op.cells.length * (14 + 12);
				...
				onLoad(hex, pos, op.cells.length);
			} )
			.fail ( onError );
