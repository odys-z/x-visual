...
class GlUniform {
	...
	static init(visual, obj3, light, ecs) {
		...
		if (visual.shader === ShaderFlag.colorArray) {
			...
			if (visual.paras && visual.paras.u_texbump)
				uniforms.u_texbump  = { value: new Vector2(...visual.paras.u_texbump) };
			else
		...
		if ( visual && visual.paras && visual.paras.envMap ) {
			...
			if (visual.paras.xColor) {
				eref = ecs.getEntity(visual.paras.xColor)
				uniforms.xColor = { value: visual.}
			}
		}
		return uniforms;
	}
