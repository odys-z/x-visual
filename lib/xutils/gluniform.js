import { Vector3, Vector4, Vector2, FrontSide } from '../../packages/three/three.module-MRTSupport';
...
class GlUniform {
	 ...
	 * @function
	 */
	static init(visual, obj3, light, ecs) {
		...
		if (visual.paras && visual.paras.color) {
			uniforms.u_color = {value: new Vector4(...visual.paras.color)};
		}

		uniforms.side = {value: !visual.paras || visual.paras.side === undefined
								? FrontSide : visual.paras.side};
		...
		if (visual.shader === ShaderFlag.colorArray) {
			...
			if (visual.paras && visual.paras.colors) {
				...
				for (var i = 0; i < visual.paras.colors.length; i++)
					uniforms[`u_colors${i}`] = {value: new Vector4(...visual.paras.colors[i])};
			}
		}
		else if (visual.shader === ShaderFlag.texEnv) {
			texEnv.initUniform(uniforms, light, visual.paras);
			...
			uniforms.u_exposure = { value: visual.paras && visual.paras.envExposure !== undefined
					? visual.paras.envExposure : 0.1 };

			if (visual.paras && visual.paras.u_texNoise)
				uniforms.u_texNoise  = { value: AssetKeepr.loadTexure(visual.paras.u_texNoise) };
			if (visual.paras && visual.paras.u_noisiness)
				uniforms.u_noisiness  = { value: new Vector4(...visual.paras.u_noisiness) };
			else
				uniforms.u_noisiness  = { value: new Vector4(0) };
		}
			...
			var wpos = visual.paras.wpos
				? new Vector3(...visual.paras.wpos) : new Vector3(0, 0, 0);
			...
			if (visual.paras.tile && visual.paras.tile.hue) {
				uniforms.u_hue = { value: new Vector4(...visual.paras.tile.hue) };
			}
			else
		...
		if ( visual && visual.paras && visual.paras.envMap ) {
			uniforms.whiteAlpha = { value: visual.paras.whiteAlpha === undefined
						? 0 : visual.paras.whiteAlpha };
			// flip x for cube texture, none for equirectangle
			let eref = ecs.getEntity(visual.paras.envMap)
			if (!eref) throw new XError('entity not found: ', visual.paras.envMap);
			uniforms.flipEnvMap = {value: eref.Obj3.datum.flipx !== undefined ?
						eref.Obj3.datum.flipx : -1 };

			uniforms.lod = { value: visual.paras.lod
						? new Vector2(...visual.paras.lod) : new Vector2(0, 0) };

			// if (visual.paras.u_tex) {
			// 	uniforms.u_tex0 = { value: AssetKeepr.loadTexure(visual.paras.u_tex[0]) };
			// 	uniforms.u_tex1 = { value: AssetKeepr.loadTexure(visual.paras.u_tex[1]) };
			// }
		}
		return uniforms;
	}
