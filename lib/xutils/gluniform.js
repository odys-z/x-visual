...
class GlUniform {
	...
	static init(visual, obj3, light) {
		...
		if (visual.paras && visual.paras.uniforms && visual.paras.uniforms.u_tex) {
			console.warn("formatUniforms(): ignoring wrong paras: Visual.paras.uniforms.u_tex = ", visual.paras.uniforms.u_tex, "Should been used asVisual.paras.u_tex.");
			visual.paras = Object.assign(visual.paras || {}, {
				u_tex: visual.paras.uniforms.u_tex
			});
		...
		if (visual.paras && visual.paras.u_tex) {
			...
			uniforms.u_tex = {
				value: AssetKeepr.loadTexure(visual.paras.uniforms.u_tex)
			};
			delete visual.paras.uniforms.u_tex;
		} else if ((visual.shader & ShaderFlag.defaultex) === ShaderFlag.defaultex) {
			uniforms.u_tex = {
				value: AssetKeepr.cheapixelTex()
			};
		}
		...
		if (visual.paras && visual.paras.color) {
			uniforms.u_color = {
				value: new THREE.Vector4(...visual.paras.color)
			};
		}

		uniforms.side = {
			value: !visual.paras || visual.paras.side === undefined ?
				THREE.FrontSide : visual.paras.side
		};
		uniforms.u_alpha = {
			value: visual.paras !== undefined && visual.paras.tex_alpha !== undefined ?
		...
		if (!uniforms.u_tex &&
			...
			if (visual.asset)
				uniforms.u_tex = {
					value: AssetKeepr.loadTexure(visual.asset)
				};
			else
				uniforms.u_tex = {
					value: AssetKeepr.cheapixelTex()
		...
		if (visual.shader === ShaderFlag.colorArray) {
			...
			phongMorph2.initUniform = phongMorph2.initUniform || texPrism.initUniform;
			phongMorph2.initUniform(light, uniforms, visual.paras);

			if (visual.paras && visual.paras.colors) {
				// TODO should we use uniform array ?
				for (var i = 0; i < visual.paras.colors.length; i++)
					uniforms[`u_colors${i}`] = {
						value: new THREE.Vector4(...visual.paras.colors[i])
					};
			}
		} else if (visual.shader === ShaderFlag.scaleOrb) {
			uniforms.wpos = {
				value: visual.paras.wpos ?
					new THREE.Vector3(...p) : new THREE.Vector3(0, 0, 0)
			};
			uniforms.r = {
				value: visual.paras.orbR === undefined ?
					20 : visual.paras.orbR
			};
			uniforms.whiteAlpha = {
				value: visual.paras.whiteAlpha === undefined ?
					0 : visual.paras.whiteAlpha
			};
			var os = visual.paras.orbScale;
			uniforms.orbScale = {
				value: os === undefined ?
					new THREE.Vector3(1, 0.2, 0.2) : new THREE.Vector3(os[0], os[1], os[2])
			};
			...
			uniforms.tmin = {
				value: visual.paras.t_range ? visual.paras.t_range[0] : 0
			};
			uniforms.tmax = {
				value: visual.paras.t_range ? visual.paras.t_range[1] : 1
			};
			...
			var rs = [];
			var orbRs = typeof visual.paras.orbR === 'number' ? [visual.paras.orbR] : visual.paras.orbR || [10];
			for (var r of orbRs) {
				rs.push(r);
			}
			var orbColors = [];
			...
			if (visual.shader === ShaderFlag.tiledOrbs) {
				...
				uniforms.speedVert = {
					value: new Array(groups).fill(0.01)
				};
				uniforms.speedFrag = {
					value: new Array(groups).fill(0.01)
				};
				...
				var colrs = new Array();
				var pcolrs = visual.paras.thermalColors || [
					[1, 0, 0],
					[0, 1, 0],
					[0, 0, 1]
				];
				...
				uniforms.thermalColors = {
					value: colrs
				};
				uniforms.maxHeight = {
					value: visual.paras.geostyle.maxHeight ||
			...
			if (visual.shader === ShaderFlag.texPrism)
				texPrism.initUniform(light, uniforms, visual.paras);
			else
				xyzLayer2.initUniform(light, uniforms, visual.paras);
			...
			uniforms.u_offsetxyz = {
				value: new THREE.Vector3(...offsetxyz)
			};
			...
			uniforms.speedVert = {
				value: new Array(groups).fill(0.001)
			};
			uniforms.speedFrag = {
				value: new Array(groups).fill(0.001)
			};
			...
			if (visual.paras.u_tex) {
				uniforms.u_tex = {
					value: AssetKeepr.loadTexure(visual.paras.u_tex[0])
				};
				uniforms.u_basetex = {
					value: AssetKeepr.loadTexure(visual.paras.u_tex[1])
				};
			}

			if (visual.paras.tile && visual.paras.tile.hue) {
				uniforms.u_hue = {
					value: new THREE.Vector4(...visual.paras.tile.hue)
				};
			} else
				uniforms.u_hue = {
					value: new THREE.Vector4(0.4, 0.2, 0.8, 1.0)
				};

			uniforms.u_north = {
				value: visual.paras.north || 0
			};
		}
	 ...
	 * @function
	 */
	static update(uniforms, light, vparas) {
		// check uniforms exists
		// update uniforms in light if light.dirty = true;
		//update common uniform value
		if (typeof vparas.paras === "object") {

			let paras = vparas.paras;
			if (paras.side !== undefined || paras.tex_alpha !== undefined) {
				uniforms.side.value = paras.side;
			}
			if (paras.tex_alpha !== undefined) {
				uniforms.u_alpha.value = paras.tex_alpha;
			}

		}

		if (vparas.shader !== undefined) {
			let shader = vparas.shader;
			let paras = vparas.paras;
			if (shader === ShaderFlag.colorArray) {

				phongMorph2.updateUniform(light, uniforms, paras);

				if (paras.colors) {
					// TODO should we use uniform array ?
					for (let i = 0; i < paras.colors.length; i++)
						uniforms[`u_colors${i}`] = {
							value: new THREE.Vector4(...visual.paras.colors[i])
						};
				}
				...
				if (paras.orbR !== undefined) {
					uniforms.r.value = paras.orbR
				}
				if (paras.whiteAlpha !== undefined) {
					uniforms.whiteAlpha = paras.whiteAlpha;
				}
				if (paras.orbScale !== undefined) {
					let os = paras.orbScale;
					uniforms.orbScale.value = new THREE.Vector3(os[0], os[1], os[2]);
				}

			} else if (shader === ShaderFlag.orbGroups) {

				if (Array.isArray(paras.offsets) && paras.offsets.length > 0) {
					let offset = paras.offsets;
					let orbs = paras.offsets.length;
				...
				if (Array.isArray(paras.orbR) && paras.orbR.length > 0) {
					let rs = [];
					for (let r of paras.orbR) {
						rs.push(r);
					}
					uniforms.r.value = rs;
				...
				if (Array.isArray(paras.colors) && paras.colors.length > 0) {
					let orbColors = [];
					for (let c of paras.colors) {
						orbColors.push(new THREE.Vector3(...c));
					}
				...
				if (paras.t_range !== undefined) {
					uniforms.tmin.value = paras.t_range[0];
					uniforms.tmax.value = paras.t_range[1];
				}

				if (Array.isArray(paras.follows) && paras.follows.length > 0) {
					let wpos = [];
					let wtan = [];
					let flws = [];
					for (let follow of paras.follows) {
						wpos.push(new THREE.Vector3(0));
						wtan.push(new THREE.Vector3(1, 0, 0));
						flws.push(follow);
					}
				...
				if (paras.whiteAlpha !== undefined) {
					uniforms.whiteAlpha.value = paras.whiteAlpha;
				}
				if (paras.orbScale !== undefined) {
					let os = paras.orbScale;
					uniforms.orbScale.value = new THREE.Vector3(os[0], os[1], os[2]);
			...
			if (shader === ShaderFlag.tiledOrbs) {
				...
				if (typeof paras.geostyle.groups === "number" && paras.geostyle.groups >= 1) {
					let groups = paras.geostyle.groups;
					uniforms.speedVert.value = new Array(groups).fill(0.01);
					uniforms.speedFrag.value = new Array(groups).fill(0.01);
				}

				if (Array.isArray(paras.thermalColors) && paras.thermalColors.length > 0) {

					var colrs = new Array();
					var pcolrs = paras.thermalColors
					for (var c of pcolrs) {
						colrs.push(new THREE.Vector3(...c));
					}
				...
				if (paras.geostyle.maxHeight !== undefined) {
					uniforms.maxHeight.value = paras.geostyle.maxHeight;
				}
				...
				if (shader === ShaderFlag.texPrism)
					texPrism.updateUniform(light, uniforms, paras);
				else
					xyzLayer2.updateUniform(light, uniforms, paras);

				if (Array.isArray(paras.wpos) && paras.wpos.length > 0) {
					uniforms.wpos = new THREE.Vector3(...visual.paras.wpos);
				}

				// xz, xy, yz layers offset
				//var offsetxyz = [0, 0, 0];
				let offsetxyz = uniforms.u_offsetxyz.value;
				if (paras.yztile !== undefined) {
					offsetxyz.x = paras.yztile.x0;
				}
				if (paras.xztile !== undefined) {
					offsetxyz.y = paras.xztile.y0;
				}
				if (paras.xytile !== undefined) {
					offsetxyz.z = paras.xytile.z0;
				}

				if (typeof paras.geostyle === "object" && paras.geostyle.groups >= 1) {
					let groups = paras.geostyle.groups;
				...
				if (paras.tile && paras.tile.hue) {
					uniforms.u_hue.value = new THREE.Vector4(...paras.tile.hue);
				}
				if (paras.north !== undefined) {
					uniforms.u_north.value = paras.north
				}
