	...
	return {
		fragmentShader: (`${glx.mrt.f_layout}
			...
			float bumpSmooth(sampler2D texBump, vec2 uv, vec4 scale) {
				...
				vec4 col2 = texture(texBump, uv + 0.01);
				vec3 nor = cross(col.rgb, col2.rgb);
				float lump = length(col);
				nor += vec3( dFdx(lump), 0.0, dFdy(lump) );
				return clamp( 15. * dot(nor.xyz, scale.xyz * -scale.w), -1.0, 1.0 );
			}
			...
			void main() {
				...
				`if (vxzWeight > 0.9) {
					...
					white = ${roofAlpha};
					lodxy = lod.x;
					bmp = bumpSmooth(u_tex0, fractuv(vUv), vec4(-0.7, 1. / (.1 + u_texbump.x), 0.7, u_texbump.x)) * vP.w;
				}`
				+ // wall
				`else {
					...
					white = ${sideAlpha};
					lodxy = lod.y;
					bmp = bumpSmooth(u_tex1, fractuv(vUv), vec4(-0.7, 1. / (.1 + u_texbump.y), 0.7, u_texbump.y)) * vP.w;
				}

				vec3 i = cameraPosition - vP.xyz;
				vec3 rit = cross(normalize(i), vNormal) * vP.w;
				vec3 v = normalize( reflect( -i , vNormal ) - rit * bmp );
		...
		vertexShader: (`#version 300 es
			...
			void main() {
				...
				// FIXME building don't need reverting normal?
