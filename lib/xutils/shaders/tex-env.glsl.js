...
export function texEnv(vparas = {}) {
	...
	return { fragmentShader: `${glx.mrt.f_layout}
		...
		void main() {
			...
			pc_FragColor = phongLight(vNormal, u_lightPos, cameraPosition, vP.xyz,
					u_ambientColor.xyz, pc_FragColor.xyz, u_specularColor.xyz, u_shininess);

			vec3 i = cameraPosition - vP.xyz;
			vec3 v = normalize(reflect( -i , vNormal ) + bmp * 4.5 * vP.w);

			float fsnl = fresnel(v, i);
			pc_FragColor.xyz *= bmp / (1. + fsnl) * vP.w;
			...
			// envColor.xyz *= length(fresnel( v, -i, lodxy ));
			+ `
			envColor.xyz *= fsnl;
			...
			xColor = pc_FragColor;
			xEnvSpecular += threshold(pc_FragColor, u_bloomThresh + 0.5);
		}
		`.replaceAll(/\n\t\t/g, '\n').replaceAll(/\t/g, '  '),
