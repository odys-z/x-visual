...
export function texEnv(vparas = {}) {
	...
	return { fragmentShader: `
		...
		vec3 texRough(sampler2D txr, vec2 uv, float scale, float bmp) {
			...
			// return noise * pow(
			// 	// texRough(u_texRough, vUv, scale * 1.3, bmp),
			// 	1. - clamp( abs(texture2D( txr, fractuv(uv * scale * 1.3) )).xyz * bmp, 0., 1.),
			// 	noise);
			return noise;
		}

		void main() {
			vec3 noise = texRough(u_texRough, vUv, 11., 0.5);
			...
			// gl_FragColor.xyz *= vIntense / white;
			gl_FragColor.xyz *= 2.5 * vIntense;

			vec3 p = vP.xyz; // + 10. * noise;
			vec3 vReflect = reflect( p - cameraPosition , vNormal );
			vec4 envColor = textureCube( envMap, vec3( flipEnvMap * vReflect.x, vReflect.yz ), lod.y );
			// vec4 envColor = textureCubeLodEXT( envMap, vec3( flipEnvMap * vReflect.x, vReflect.yz ), lod.y );
			`
			// why so narrow ? show we use environment version?
			// envColor.xyz = bsdfGGX(vec3(300.) - vP.xyz, cameraPosition - vP.xyz, vNormal, envColor.xyz, 0.8);
			+ `
			envColor.xyz = bsdfGGX(vReflect, cameraPosition - p, vNormal, envColor.xyz, 0.2);
						// fresnel(vReflect, cameraPosition - p, 0.6));

			vec3 toneColr = mixColorT(gl_FragColor.xyz, envColor.xyz, white);
			// vec3 toneColr = mixColorT(gl_FragColor.xyz, envColor.xyz, white) * .3;
