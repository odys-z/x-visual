		...
		return `
			...
			vec3 bokeh(sampler2D bokehDepth, sampler2D tColor, vec2 uv) { ${ debug ? `
			  if ( showFocus &&
				...
				return vec3(0., 1., 0.);` : '' }
			  float s2 = linearize(texture(bokehDepth, uv).z);
			  float s1 = linearize(texture(bokehDepth, focusCoords).z);
			  float blur = 0.0;
			  if (manualdof) {
			    float a = s2 - s1;
			  ...
			  if(blur < CoC * 0.1) {
			    ...
			    if (showFocus) col.r = blur * 10.;
			  }
			  else {
				col = textureLod(xFragColor, uv.xy, blur * 100.).rgb;
