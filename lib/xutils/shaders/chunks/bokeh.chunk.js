		...
		return `
			...
			vec3 bokeh(sampler2D bokehDepth, sampler2D tColor, vec2 uv) { ${ debug ? `
			  ...
			  if (manualdof) {
				...
				blur = c;
				blur = smoothstep(0., CoC, c);
			  }

			  vec3 col = vec3(0.0);

			  col = texture( xFragColor, uv.xy, blur * B ).rgb;

			  float thresh = CoC * 0.3333;

			  // front blur
			  us2 = textureLod( bokehDepth, uv, B ).z;

			  if (blur < thresh && us2 < us1) {
				// us2 /= us1 - us2;
				s2 = linearize(us2) * 1000.0;

				float m = f / (s1 - f);
				float c = abs(1. - s1 / s2) * A * m;

				blur = smoothstep(CoC, B, c);
				// vec4 col1 = textureLod(xFragColor, uv, blur * A * thresh * B );
				vec4 col1 = textureLod(xFragColor, uv, B );
				col = col1.rgb; // mix(col, col1.rgb, delta);

				if ( showFocus ) col.r = 1.;
			  }

			  if ( showFocus && blur < thresh ) col.b = 1.;
