...
export const mrt = {
	...
	f_blurH: `
		vec4 blurH( in sampler2D tex, in vec2 uv, in vec2 blurSize, in float t0 ) {
			...
			sum += threshold( textureLod(tex, vec2(uv.x - 4.0 * blurSize.x, uv.y), 3.), t0) * 0.05;
			sum += threshold( textureLod(tex, vec2(uv.x - 3.0 * blurSize.x, uv.y), 3.), t0) * 0.09;
			sum += threshold( textureLod(tex, vec2(uv.x - 2.0 * blurSize.x, uv.y), 3.), t0) * 0.12;
			sum += threshold( textureLod(tex, vec2(uv.x - 1.0 * blurSize.x, uv.y), 3.), t0) * 0.15;
			sum += threshold( textureLod(tex, uv, 3.), t0) * 0.16;
			sum += threshold( textureLod(tex, vec2(uv.x + 1.0 * blurSize.x, uv.y), 3.), t0) * 0.15;
			sum += threshold( textureLod(tex, vec2(uv.x + 2.0 * blurSize.x, uv.y), 3.), t0) * 0.12;
			sum += threshold( textureLod(tex, vec2(uv.x + 3.0 * blurSize.x, uv.y), 3.), t0) * 0.09;
			sum += threshold( textureLod(tex, vec2(uv.x + 4.0 * blurSize.x, uv.y), 3.), t0) * 0.05;
