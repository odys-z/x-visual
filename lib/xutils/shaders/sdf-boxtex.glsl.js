...
export function cubeTex(vparas = {}) {
 ...
 return { fragmentShader: `
  ...
  vec3 innerCube(vec3 mpos, vec3 box, vec3 cubes) {
    return vec3(-0., 0., 0.);
  }

  vec3 outerCube(vec3 cube, vec3 size) {
  ...
  vec4 mainImage( in vec2 fragCoord ) {
    ...
    float d3 = box3( vP.xyz - cube, vsize * 0.5, 0. ); // d ( cube(0, 0, 0) )
    if (d3 <= 0.) {
	  col = 1.0;
	  ...
	  col += 4. * line(e, vP.xyz - cube, vec3(0., -10., 0.), vec3(50., -10, 0.), WEIGHT * 50.);
    }
	else col = .4;
