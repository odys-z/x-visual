...
import {GlxError, glx} from './glx.glsl'
...
export function phongMorph2(vparas) {
  ...
  vertexShader: `
    ...
    void main(){
        ...
        vec3 diffuseColor = morphColors();
        diffuseColor = mix(diffuseColor, u_color.rgb, intenseAlpha(u_lightIntensity));

        vec3 N = normalize(normalInterp);
        vec3 L = normalize(u_lightPos - vertPos);

        vec2 lambershine = lambertShine(N, L, vertPos, u_shininess);
        vColor = vec4(Ka * u_ambientColor +
                      Kd * lambershine.s * diffuseColor +
