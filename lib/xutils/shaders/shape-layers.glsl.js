...
export function shapeLayers(vparas) {
 ...
 return { fragmentShader: `
  ...
  float boxY(vec3 e, vec3 P, vec3 c0, vec2 size, float w) {
	...
	float d = 0.;
	if (p0d.w > 0.)
      d = box( vec2(p0d.x, p0d.z), size * 0.5, 5. );
	else return 0.;
  ...
  vec4 mainImage( in vec2 fragCoord ) {
    // work with pointFace()
    // float col = boxY(cameraPosition, P, P0, vec2(100., 100.), WEIGHT);
    // col += boxY(cameraPosition, P, P2, vec2(100., 100.), WEIGHT);
