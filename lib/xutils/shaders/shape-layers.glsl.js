...
export function shapeLayers(vparas) {
 ...
 return { fragmentShader: `
  ...
  vec4 mainImage( in vec2 fragCoord ) {
    ...
    vec2 v0 = P0.xy;
    vec2 v1 = P1.xy;
    float col = line(vUv.xy, v0, v1, line_width);
    return vec4(col);
  }
 ...
 vertexShader: `
  ...
  void main() {
    ...
    vec4 p4 = projectionMatrix * modelViewMatrix * vec4( position + vec3(-150., 0., -position.z), 1.0 );
    P0 = p4.xyz;
    p4 = projectionMatrix * modelViewMatrix * vec4( position + vec3(150., 0., -position.z), 1.0 );
    P1 = p4.xyz;
    p4 = projectionMatrix * modelViewMatrix * vec4(uv.s, uv.t, 2., 1.0);
    vUv = p4.xyz; // / p4.z;
  } `
 }
}
