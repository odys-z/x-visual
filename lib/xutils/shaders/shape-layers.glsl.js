...
export function xyzLayer2(vparas) {
 var {xytile, yztile, xztile} = para4Layers(vparas);
 ...
 return { fragmentShader: `
  ...
  float box2Color(vec3 i, vec3 c0, vec3 n0, vec3 radi, vec2 size, vec4 tiles, float w) {
    vec4 p0 = rayPlaneInsec( e, i, c0, n0 );

    if (p0d.w > 0.) {
      float tes = tessellate2( p0d.xz, c0.xz, size * 0.5 / tiles );

      vec2 pxz = rotate2(radi, p0d.xz - c0.xz);
  ...
  vec4 mainImage( in vec2 fragCoord ) {
	...
	for (int cx = 0; cx < ${layrs}; cx++) {
      // col += boxY(cameraPosition, P, vcent[cx].xyz, vsize, ${vtiles} * 0.5, WEIGHT);
      col += box2Color(i, vcent[cx].xyz, vec3(0., 1., 0.), radi, vsize, ${vtiles} * 0.5, WEIGHT);
      col += box2Color(i, vcent[cx].yxz, vec3(1., 0., 0.), radi, vsize, ${vtiles} * 0.5, WEIGHT);
      col += box2Color(i, vcent[cx].zyx, vec3(0., 0., 1.), radi, vsize, ${vtiles} * 0.5, WEIGHT);
    }
	return u_hue * col;
  }
...
function para4Layers(vparas) {
    ...
    return {xytile: tile(vparas.xytile || {}),
    ...
    function tile(ptile) {
        ...
        return {layrs, vtiles, edgeWeight, boxsize, tiles,
                floorHeightDeclare, floorHeightRef};
    }
}
