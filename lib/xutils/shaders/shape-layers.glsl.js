...
export function texPrism(vparas = {}) {
 ...
 return { fragmentShader: `
  ...
  vec4 texY(vec3 e, vec3 P, vec3 c0, vec2 size, vec2 tiles, vec2 boader, sampler2D tex) {
    ...
    if (p0d.w > 0.) {
      ...
      tes = 0.4/tes * (1. - va) * ( 1. - abs( sin(now * 0.0005) ) );
      vec4 col4 = vec4( abs(box) * tes + abs(box) * 0.02 );
    }
    else return vec4(0.);
  }
  ...
  vec4 mainImage( in vec2 fragCoord ) {
    ...
    // upward base (roof)
    vec4 col4 = vec4(0., col * 0.2, 0.8, col);
    col4 += texY(cameraPosition, P, cent[${layrs} - 1].xyz, vsize, vtiles * 0.5, vec2(WEIGHT), u_tex[0]);
    return col4;
  }
 ...
 vertexShader: `
  ...
  attribute vec2 a_tiles; // x-div, layer, z-div
...
export function texPrism_box2(vparas) {
...
export function texPrism_line_dist(vparas) {
 var layrs = vparas.layars || 3;
 return { fragmentShader: `
  #define WEIGHT 0.4
