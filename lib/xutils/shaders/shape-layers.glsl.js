...
export function texPrism(vparas = {}) {
 ...
 // FIXME sampler2D arry can't work, but why colorArray works?
 return { fragmentShader: `
  ...
  void main() {
    ...
    gl_FragColor.a += va;
    if (vxzWeight <= 0.9) {
        gl_FragColor = mix(gl_FragColor, texture2D(u_tex1, mod(vUv, 1.)),
		${sideAlpha});
    }
  }`,
...
export function xyzLayer2(vparas) {
 ...
 return { fragmentShader: `
  ...
  // box 2d rasterize
  // float box2Color(vec3 e, vec3 i, vec3 c0, vec3 n0, float radi, vec2 size, vec2 tiles, float w) {
  //   // i = rotateY(radi, i);
  //   // e = rotateY(radi, e);
  ...
  //       p2d = p_.yz;
  //
  //     float tes = tessellate2( p2d, vec2(0.), size / tiles );
  //     float box = box2( p2d, size * 0.5, 0.5 );
  //
  //     box = 1.0/box * w * (1. - va);
  //
  ...
  float box3(vec3 p, vec3 size, float r){
  ...
  vec4 mainImage( in vec2 fragCoord ) {
    ...
    // normalize north for box cutting
    // vec3 i = normalize(rotateY(vnorth, P - cameraPosition));
    // vec3 e = rotateY(vnorth, cameraPosition);

    vec3 i = normalize(P - cameraPosition);
    vec3 e = cameraPosition;

	vec3 size3 = vec3(160., 240., 160.);
    for (int cx = 0; cx < ${yztile.layrs}; cx++) {
      vec3 ci = vcent.xyz + vec3(float(cx) * ${yztile.layerDist} + vxyzOffset.x, 0., 0.);
      // col += box2Color(e, i, ci, vec3(1., 0., 0.), vnorth, vsize[1], vtiles[1], WEIGHT);
      col += box3Color(e, i, ci, vec3(1., 0., 0.), vnorth, size3, vtiles[1], WEIGHT);
    }

    for (int cx = 0; cx < ${xztile.layrs}; cx++) {
      vec3 ci = vcent.xyz + vec3(0., float(cx) * ${xztile.layerDist} + vxyzOffset.y, 0.);
      // col += box2Color(e, i, ci, vec3(0., 1., 0.), vnorth, vsize[2], vtiles[2], WEIGHT);
      col += box3Color(e, i, ci, vec3(0., 1., 0.), vnorth, size3, vtiles[2], WEIGHT);
    }

    for (int cx = 0; cx < ${xytile.layrs}; cx++) {
      vec3 ci = vcent.xyz + vec3(0., 0., float(cx) * ${xytile.layerDist} + vxyzOffset.z);
      // col += box2Color(e, i, ci, vec3(0., 0., 1.), vnorth, vsize[0], vtiles[0], WEIGHT);
      col += box3Color(e, i, ci, vec3(0., 0., 1.), vnorth, size3, vtiles[0], WEIGHT);
    }
 ...
 vertexShader: `
  ...
  void main() {
    ...
    // vcent = vec4(vec3(a_loc.x, -a_box.y, a_loc.y) + wpos, 1.);
