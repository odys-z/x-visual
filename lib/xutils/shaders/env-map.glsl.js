	...
	return {
		...
		vertexShader: `#version 300 es
			...
			void main() {
				...
				${bokeh ? `//vzBokeh = (viewMatrix * vP).z;
				vec4 mvPosition = modelViewMatrix * vec4(position, 1.0);
				vzBokeh = - mvPosition.z;
				` : ''}
			}
		`.replaceAll(/\n\t\t\t/g, '\n')
		.replaceAll(/\t/g, '  '),
		...
		fragmentShader: `${glx.mrt.f_layout}
			...
			${bokeh ? glx.bokehDepth.f : ''}
			${glx.equirectUv}
			${glx.threshold}

			void main() {
				vec3 i = normalize( cameraPosition - vP.xyz );
				...
				${bokeh ? 'xBokehDepth = bokehDepth()' :
						// else no color at infinite far
						// 'xBokehDepth = vec4(.0, .0, 1., 0.)'
						'' };
			}
		`.replaceAll(/\n\t\t\t/g, '\n')
		.replaceAll(/\t/g, '  ')
