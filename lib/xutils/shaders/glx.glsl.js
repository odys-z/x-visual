...
export const glConfig = {
  ...
  ka: '(0.3)',   // Ambient reflection coefficient, e.g. 1.0,
  kd: '(1.0)',   // Diffuse reflection coefficient, e.g. 0.4,
  ks: '(0.8)',   // Specular reflection coefficient, e.g. 1.0,
...
const glx = {
  ...
  phongLight: `vec2 lambershine(vec3 n, vec3 lightpos, vec3 e, vec3 vertpos, float shininess) {
	...
	vec3 N = normalize(n);
    float lambertian = max(dot(n, normalize(lightpos)), 0.0);
    vec3 R = reflect(-L, N);
    float specAngle = max(dot(R, normalize(e - vertpos)), 0.0);
    float specular = pow(specAngle * ${glConfig.ks}, shininess);
    return vec2(lambertian, specular * lambertian);
  }
  vec4 phongLight(vec3 N, vec3 lightpos, vec3 eye, vec3 vertpos,
      vec3 ambient, vec3 diffuse, vec3 specular, float shininess) {
    float Ka = ${glConfig.ka};
    float Kd = ${glConfig.kd};
    float Ks = ${glConfig.ks};

    vec2 lambshine = lambershine(N, lightpos, eye, vertpos, shininess);
