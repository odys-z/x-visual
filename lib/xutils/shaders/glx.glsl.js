...
const glConfig = {
	...
	*/
	...
	xbuffer: { w: 1024, h: 1024, },

	/** filter quad buffer size */
	xfilter: { w: 1024, h: 1024, }
}
...
const glx = {
  ...
  fresnelAlpha: `float fresnelAlpha(vec3 e, vec3 P, vec3 np) {
	...
	// a typical fresnel can be: https://www.scratchapixel.com/lessons/3d-basic-rendering/introduction-to-shading/reflection-refraction-fresnel
	// three.js's Schlick functions are roughness & specular color dependent on fresnel.
	/*
	fresnel: `vec3 fresnel(vec3 incident, vec3 viewDir, float r) {
				incident = normalize(incident) ;
				vec3 halfDir = normalize( incident + normalize(viewDir) );
				float dotLH = clamp( dot( incident, halfDir ), 0., 1. );
				float fresnel = exp2( ( -5.55473 * dotLH - 6.98316 ) * dotLH );
				vec3 Fr = max( vec3( 1.0 - r ), incident ) - incident;
				return Fr * fresnel + incident;
			}`,
	*/
	fresnel: `float fresnel(vec3 incident, vec3 viewDir) {
				incident = normalize(incident) ;
				vec3 halfDir = normalize( incident + normalize(viewDir) );
				float dotLH = clamp( dot( incident, halfDir ), 0., 1. );
				float f = exp2( ( -5.55473 * dotLH - 6.98316 ) * dotLH );
				return f;
			}`,

	fresnel_del: `float fresnel(vec3 incident, vec3 viewDir, float r) {
				incident = normalize(incident) ;
				vec3 halfDir = normalize( incident + normalize(viewDir) );
				float dotLH = clamp( dot( incident, halfDir ), 0., 1. );
				float fresnel = exp2( ( -5.55473 * dotLH - 6.98316 ) * dotLH );
				return ( 1.0 - r ) * fresnel + r;
			}`,
