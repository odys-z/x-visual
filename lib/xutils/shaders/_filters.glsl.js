		...
		fragmentShader: (`#version 300 es
			...
			void main() {
				...
				${debug && bokeh ? 'xBlurColor = texture( xBokehDepth, vUv );'
								 : '' }
				${!debugSSAO && ssao  ? 'xBlurColor.rgb *= 1.0 - ssao( xBokehDepth, vUv );' : ''}
...
const filters = {
	...
	*/
	...
	ssao: paras => (`uniform float u_ssaoRadius;
		uniform float u_ssaoIntense;
		float ssao(sampler2D tex, vec2 uv) {
			float epsilon = float(${paras.epsilon || 0.03}) * max(0.0001, us2);
			float epsilon_10 = epsilon * 0.01;
			float lod = float(${paras.lod || 4.0}) / (1. + us2 * (float(${paras.lod || 4.0}) - 1.0) );
			vec2 r = u_ssaoRadius / u_texsize / max(0.001, us2);
			float us = 0.;
			float w = 0.;
			float us_0 = texture( tex, uv, lod ).z;
			float d = -Pi_10;
			d = atan(dFdx(us2), dFdy(us2));
			if (us_0 < us2 && us2 - us_0 < epsilon) {
				vec2 sampleuv = uv + vec2(cos(d), sin(d)) * r;
				float us_ = texture( tex, sampleuv ).z;
				if (us_ < us2 - epsilon_10 && us2 - us_ < epsilon) {
					w++;
				}
				...
				if (w > 0.) {
					float occ = w / 3. * (us2 - us_0);
					return max(occ, w) * u_ssaoIntense;
				}
			}
	...
	ssao_bak: (`uniform float u_ssaoRadius;
		uniform float u_ssaoIntense;
		float ssao(sampler2D tex, vec2 uv) {
			float epsilon = 0.01 * max(0.0001, us2);
