...
export function _filters(xwparas = {}, debug) {
	...
	let ssao = pFinal.ssaoRadius > 0 && pFinal.ssaoIntense > 0;
		...
		fragmentShader: (`#version 300 es
			...
			void main() {
				...
				${debug && bokeh ? 'xBlurColor = texture( xBokehDepth, vUv );'
								 : '' }
				${ssao  ? 'xBlurColor.rgb -= ssao( xBokehDepth, vUv );' : ''}
				${debug && ssao ? 'xOutline = vec4(0., 0., ssao( xBokehDepth, vUv ), 1.);'
								: 'xOutline += vec4(0.5);' }
	...
	*/
	...
	ssao: (`uniform float u_ssaoRadius;
		...
		float ssao(sampler2D tex, vec2 uv) {
			float epsilon = 0.1 / u_ssaoIntense * (0.1 + us2);`
			// FIXME epsilon should been scaled to camera frustum
			+ `
			float lod = 1.; //u_ssaoRadius / (1. + us2);
			vec2 r = u_ssaoRadius / u_texsize / (1. + us2);
			// float r = u_ssaoRadius;
			float d = -Pi_10;
			float us = 0.;
			float w = 0.;
			float us_ = texture( tex, uv, lod ).z;
			if (us_ < us2 && us2 - us_ < epsilon) {
				us_ = textureLod( tex, uv + vec2(cos(d), sin(d)) * r, lod).z;
				if (us_ < us2) {
					us += us_; w++;
				}

				us_ = textureLod( tex, uv + vec2(cos(d + Pi2_3), sin( d + Pi2_3)) * r, 0.).z;
				if (us_ < us2) {
					us += us_; w++;
				}
	...
	ssao_bak: (`uniform float u_ssaoRadius;
		...
		float ssao(sampler2D tex, vec2 uv) {
			...
			// FIXME epsilon should been scaled to camera frustum
			+ `
			float lod = 3.; //u_ssaoRadius / (1. + us2);
			vec2 r = u_ssaoRadius / u_texsize;
			// float r = u_ssaoRadius;
			float d = -Pi_10;
			// float us = textureLod( tex, uv, lod).z;
			float us = 0.;
			float w = 0.;
			float us_;
			us_ = textureLod( tex, uv + vec2(cos(d), sin(d)) * r, 0.).z;
			if (us_ < us2 && us2 - us_ < epsilon) {
				us_ = textureLod( tex, uv + vec2(cos(d), sin(d)) * r, lod).z;
				us += us_; w++;
			}
			...
			us_ = textureLod( tex, uv + vec2(cos(d + Pi2_3), sin( d + Pi2_3)) * r, 0.).z;
			if (us_ < us2 && us2 - us_ < epsilon) {
				us_ = textureLod( tex, uv + vec2(cos(d + Pi2_3), sin( d + Pi2_3)) * r, lod).z;
				us += us_; w++;
			}
