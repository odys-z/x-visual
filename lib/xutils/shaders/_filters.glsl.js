...
export function _hBlur(vparas = {}, debug) {
	...
	return {
		vertexShader: `#version 300 es
			...
			uniform vec3 u_testPos;

			in vec3 position;
			in vec2 uv;
			out vec2 vUv;

			${receiveShadow ? glx.vPShadow.setVpDirShadowcoord : ''}

			void main() {
				vUv = uv;
				gl_Position = projectionMatrix * modelViewMatrix * vec4( position, 1.0 );
				${!receiveShadow ? '' : 'setVpDirShadowcoord(directionalShadowMatrix[0], vec4(u_testPos, 1.))'};
			}
		`.replaceAll(/\n\t\t\t/g, '\n')
		.replaceAll(/\t/g, '  '),
		...
		fragmentShader: `#version 300 es
			...
			void main() {
				...
				// works, only for Texture but DataTexture: xBlurColor.rgb = texture( u_testex, vUv ).rgb;
				// works: xBlurColor.rgb = vec3(0.5);
				// works, varying while moving camera (via u_testPos): xBlurColor.rgb = abs(vP_DirShadowCoord.xyz);
				// works, showing shadow map: xBlurColor.rgb = texture( directionalShadowMap[ 0 ], vUv ).rgb * 0.75;
				+ `
				${debug ? `xBlurColor.rgb += texture( directionalShadowMap[ 0 ], vUv ).rgb * 0.4` : ''};
				xOutline += vec4(0.);
