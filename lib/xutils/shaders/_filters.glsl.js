...
const filters = {
	...
	*/
	...
	// FIXME
	// This is error for no geometry info been considered - occlusion should happened
	// in corner not depth edge
	...
	ssao: (`uniform float u_ssaoRadius;
		...
		float ssao(sampler2D tex, vec2 uv) {
			float epsilon = 0.1 * max(0.0001, 1. - us2 * us2);
			float lod = u_ssaoLOD;
			...
			if (us_ + 0.1 * epsilon < us2 && us2 - us_ < epsilon) {
				...
				return max( aov2.x / aov2.y, 0. ) * u_ssaoIntense;
			}
