	...
	return {
		...
		fragmentShader: (`#version 300 es
			...
			void main() {
				...
				${debug && ssao ? 'xOutline = vec4(1., 0., 0., 1.); //ssao( xBokehDepth, vUv ));'
								: 'xOutline += vec4(0.5);' }
	...
	*/
	...
	ssao: (`uniform float u_ssaoRadius;
		...
		float ssao(sampler2D tex, vec2 uv) {
			...
			+ `
			float lod = 0.; //u_ssaoRadius / (1. + us2);
			float d = Pi_10;
			float us = textureLod( tex, uv + vec2(cos(d), sin(d)) * u_ssaoRadius, lod).z;
			us += textureLod( tex, uv + vec2(cos(d + Pi2_3), sin( d + Pi2_3)) * u_ssaoRadius, lod).z;
			us += textureLod( tex, uv + vec2(cos(d - Pi2_3), sin( d - Pi2_3)) * u_ssaoRadius, lod).z;

			us *= .3333333333333;
			return max(us2 - us, 0.) * 10.;// * u_ssaoIntense;
		}
		`).replaceAll(/\t\t/g, '').replaceAll(/\t/g, '  '),
