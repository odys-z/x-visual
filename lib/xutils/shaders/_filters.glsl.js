...
export function _hBlur(vparas = {}, debug) {
	...
	return {
		...
		fragmentShader: (`#version 300 es
			...
			void main() {
				xBlurColor = blur( xEnvSpecular, vUv, u_blurRadius, u_texsize );
...
const filters = {
	...
	*/
	blurTripod: (`
		...
		vec4 blur( in sampler2D tex, in vec2 uv, in float blurRadius, in vec2 texsize ) {
			float thetaStep = 16.0;
			vec2 r = blurRadius / texsize;
			// vec2 r = vec2(blurRadius) * 20.;

			vec4 blor = texture(tex, uv);
			float lod = blurRadius * 1.2;

			for( float d = 0.0; d < _2Pi; d += _2Pi / thetaStep ) {
			    // blor += textureLod( tex, uv + vec2(0.0, 1.0) * r * 0.5, lod);
			    // blor += textureLod( tex, uv + vec2(-0.886, -0.5) * r * 0.6, lod);
			    // blor += textureLod( tex, uv + vec2(0.886, -0.5) * r * 0.55, lod);

			    float intense = pow(d, 0.5);
			    blor += textureLod( tex, uv + vec2(cos(d), sin(d)) * r * d, lod * d) * intense;
			    blor += textureLod( tex, uv + vec2(cos(d + Pi2_3), sin( d + Pi2_3)) * r * d, lod) * intense;
			    blor += textureLod( tex, uv + vec2(cos(d - Pi2_3), sin( d - Pi2_3)) * r * d, lod) * intense;
			}
			blor /= 3.0 * thetaStep + 1.;
			blor.a = 1.0;
			return blor;
		}`)
		.replaceAll(/\t\t/g, '').replaceAll(/\t/g, '  '),
